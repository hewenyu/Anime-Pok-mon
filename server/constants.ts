...
4.  \`imageUrl\`: (string, optional but **STRONGLY RECOMMENDED**) A direct link to a small, official-style item icon (e.g., 32x32 or 48x48 pixels). **Prioritize PokeAPI sprite URLs** (e.g., "精灵球" (Poke Ball) -> \`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/poke-ball.png\`; "伤药" (Potion) -> \`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/potion.png\`). If a suitable official URL cannot be found (e.g., for a unique custom item, or unable to map Chinese name to a known English ID), this field can be omitted. **DO NOT use placeholder URLs or broken links if you provide this field.**
