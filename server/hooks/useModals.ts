...
export const useModals = () => {
  ...
  const [selectedNPCForChat, setSelectedNPCForChat] = useState<NPC | null>(
    null
  );
  const [npcChatSuggestions, setNpcChatSuggestions] = useState<
    AIStoryChoice[] | null
  >(null);
  ...
  const toggleGlobalMapModal = useCallback(
    () => setShowGlobalMapModal(prev => !prev),
    []
  );
  const toggleNPCListModal = useCallback(
    () => setShowNPCListModal(prev => !prev),
    []
  );
  const togglePlayerProfileEditModal = useCallback(
    () => setShowPlayerProfileEditModal(prev => !prev),
    []
  );
  ...
  const openPokemonDetailModal = useCallback(
    (pokemon: Pokemon) => setPokemonToViewInModal(pokemon),
    []
  );
  const closePokemonDetailModal = useCallback(
    () => setPokemonToViewInModal(null),
    []
  );
