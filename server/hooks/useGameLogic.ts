...
import {
    GameState, GameMode, PlayerProfile, StorySegment, Pokemon,
    AIStoryResponse, AIEventTrigger, InventoryItem, AIStoryChoice,
    StoryChoice, ChatHistoryEntry, NPC, CustomizedStartData,
    AICustomizationScreenActionTag, LoadingStatus, ClassifiedIntent, CustomizationIntentType, FullProfileSuggestionData, UserDateTimeInput, ProfileDataForTimeSuggestion
} from '../types';
import {
    INITIAL_GAME_STATE,
    STORY_DATA,
...
import { fetchStoryContinuation, classifyCustomizationIntent, OnRetryCallback } from '../services/geminiService'; // Added classifyCustomizationIntent
import { sanitizePokemonData, sanitizeNPCData, sanitizeItemData } from '../utils/dataSanitizers';
...
const userDateTimeToTimestamp = (input: UserDateTimeInput): number => {
    ...
    if (input.year <= 0) { // If user entered a negative year or 0 (intending BC)
    ...
    return new Date(jsYear, jsMonth, input.day, input.hour, input.minute).getTime();
};


export const useGameLogic = () => {
  const [gameState, setGameState] = useState<GameState>(INITIAL_GAME_STATE);
  const [currentStaticSegmentId, setCurrentStaticSegmentId] = useState<string>('INITIAL_PROFILE_PREPARATION');

  const advanceGameTimeInternal = (currentTs: number, options: { minutes?: number; hours?: number }): number => {
  ...
  const updateGameState = useCallback((
    ...
    setGameState(prev => {
      ...
      if (newState.gameMode === GameMode.ADVENTURE && timeAdvanceOptions) {
        const newTime = advanceGameTimeInternal(newState.currentGameTime, timeAdvanceOptions);
  ...
  const processAIEvent = useCallback((
    ...
    let newGlobalAreaMap = { ...currentState.globalAreaMap };
    let newKnownNPCs = [...currentState.knownNPCs];
    let currentSceneToUpdate = currentState.currentAIScene ? {...currentState.currentAIScene} : {narrative: "", choices: []};
    let newPlayerProfile = {...currentState.playerProfile};
    let newAiSuggestedGameStartTime = currentState.aiSuggestedGameStartTime;


    if (event.message && currentSceneToUpdate && !isNPCContext && currentState.gameMode !== GameMode.CUSTOMIZE_RANDOM_START) {
        currentSceneToUpdate.narrative = (currentSceneToUpdate.narrative || "") + `\n\n(${event.message})`;
    }
    ...
    switch (event.type) {
      case 'SET_PLAYER_PROFILE':
        if (event.profileDetails) {
          newPlayerProfile = {
            ...newPlayerProfile,
            name: event.profileDetails.name !== undefined ? event.profileDetails.name : newPlayerProfile.name,
            gender: event.profileDetails.gender !== undefined ? event.profileDetails.gender : newPlayerProfile.gender,
            age: event.profileDetails.age !== undefined ? event.profileDetails.age : newPlayerProfile.age,
            description: event.profileDetails.description !== undefined ? event.profileDetails.description : newPlayerProfile.description,
            stamina: event.profileDetails.stamina !== undefined ? event.profileDetails.stamina : newPlayerProfile.stamina,
            maxStamina: event.profileDetails.maxStamina !== undefined ? event.profileDetails.maxStamina : newPlayerProfile.maxStamina,
            energy: event.profileDetails.energy !== undefined ? event.profileDetails.energy : newPlayerProfile.energy,
            maxEnergy: event.profileDetails.maxEnergy !== undefined ? event.profileDetails.maxEnergy : newPlayerProfile.maxEnergy,
            healthStatus: event.profileDetails.healthStatus !== undefined ? event.profileDetails.healthStatus : newPlayerProfile.healthStatus,
          };
           if (customizationScreenActionTag === 'GENERATE_FULL_RANDOM_PROFILE' || customizationScreenActionTag === 'GENERATE_RANDOM_PLAYER_DESCRIPTION') {
             if (newPlayerProfile.name === undefined) newPlayerProfile.name = "探险者";
             if (newPlayerProfile.gender === undefined) newPlayerProfile.gender = "男";
             if (newPlayerProfile.age === undefined) newPlayerProfile.age = 16;
             if (newPlayerProfile.description === undefined) newPlayerProfile.description = "一位充满好奇心的冒险者。";
             if (newPlayerProfile.stamina === undefined) newPlayerProfile.stamina = INITIAL_GAME_STATE.playerProfile.stamina!;
             if (newPlayerProfile.maxStamina === undefined) newPlayerProfile.maxStamina = INITIAL_GAME_STATE.playerProfile.maxStamina!;
             if (newPlayerProfile.energy === undefined) newPlayerProfile.energy = INITIAL_GAME_STATE.playerProfile.energy!;
             if (newPlayerProfile.maxEnergy === undefined) newPlayerProfile.maxEnergy = INITIAL_GAME_STATE.playerProfile.maxEnergy!;
             if (newPlayerProfile.healthStatus === undefined) newPlayerProfile.healthStatus = INITIAL_GAME_STATE.playerProfile.healthStatus!;
           }
        }
        break;
      case 'GIVE_ITEM':
        if (event.itemDetails) {
          ...
          const existingItemIndex = newInventory.findIndex(i => i.id === aiItem.id || i.name === aiItem.name);
      ...
      case 'START_BATTLE':
        if (event.enemyPokemonDetails) {
          ...
          return {
             ...currentState,
             playerProfile: newPlayerProfile, playerTeam: newPlayerTeam, inventory: newInventory, money: newMoney,
             currentObjective: newObjective, currentLocationDescription: newLocation, currentAreaMap: newAreaMap,
             globalAreaMap: newGlobalAreaMap, knownNPCs: newKnownNPCs,
             aiSuggestedGameStartTime: newAiSuggestedGameStartTime,
             pendingBattleDetails: {
                enemyPokemon: { ...enemyFromAI, currentHp: enemyFromAI.maxHp, isFainted: false, instanceId: `${enemyFromAI.id}-enemy-${Date.now()}` },
      ...
      case 'UPDATE_MONEY':
        ...
        } else if (event.quantity && event.quantity !== currentState.money && currentState.gameMode === GameMode.ADVENTURE) {
             newMoney = currentState.money + (event.quantity || 0);
        } else if (currentState.gameMode === GameMode.CUSTOMIZE_RANDOM_START) {
      ...
      case 'UPDATE_LOCATION':
        if (event.newLocation && typeof event.newLocation === 'string' && event.newLocation.trim() !== "") {
          ...
          newAreaMap = event.mapData;
          const currentLocKey = currentState.currentLocationDescription.trim();
      ...
      case 'ADD_POKEMON_TO_TEAM':
        if (event.pokemonDetails) {
            ...
            newPlayerTeam.push({...newPokemonFromAI, isPlayerOwned: true, currentHp: newPokemonFromAI.maxHp, isFainted: false, statusConditions: [], statStageModifiers: []});
        }
        break;
      case 'INTRODUCE_NPC':
        if (event.npcDetails) {
      ...
      case 'UPDATE_NPC_RELATIONSHIP':
        if (event.npcId && event.newRelationshipStatus) {
          const npcIndex = newKnownNPCs.findIndex(npc => npc.id === event.npcId);
          if (npcIndex > -1) {
            newKnownNPCs[npcIndex].relationshipStatus = event.newRelationshipStatus;
          }
        }
        break;
    }
    return {
        ...
        globalAreaMap: newGlobalAreaMap, knownNPCs: newKnownNPCs,
        aiSuggestedGameStartTime: newAiSuggestedGameStartTime,
        currentAIScene: isNPCContext || currentState.gameMode === GameMode.CUSTOMIZE_RANDOM_START ? currentState.currentAIScene : currentSceneToUpdate,
    };
  }, [sanitizePokemonData, sanitizeNPCData, sanitizeItemData]);

  const triggerAIStory = useCallback(async (
    playerActionTagOrInput?: string,
    isCustomizationScreenActionContext: boolean = false,
    imageRegenContext?: { pokemonInstanceIdToRegenerate: string; pokemonNameToRegenerate: string }
  ) => {
    const currentActionIsForCustomizationScreen = isCustomizationScreenActionContext ||
        (playerActionTagOrInput ? ['GENERATE_FULL_RANDOM_PROFILE', 'GENERATE_RANDOM_STARTER_POKEMON', 'GENERATE_RANDOM_ITEM', 'GENERATE_RANDOM_PLAYER_DESCRIPTION'].includes(playerActionTagOrInput) : false);

    let systemPromptToUse = GEMINI_GAME_MASTER_SYSTEM_PROMPT;
    if (playerActionTagOrInput === 'USER_REQUESTS_POKEMON_IMAGE_REGENERATION' || currentActionIsForCustomizationScreen) {
    ...
    updateGameState(prev => ({
      ...
      currentAIScene: prev.currentAIScene && playerActionTagOrInput !== 'ACKNOWLEDGE_BATTLE_START' && playerActionTagOrInput !== 'CONFIRMED_STATIC_BATTLE_FINAL' ? { ...prev.currentAIScene, choices: [], suggestedPlayerReplies: [] } : null,
      customizationAssistantResponse: currentActionIsForCustomizationScreen ? null : prev.customizationAssistantResponse,
      pokemonInstanceIdToRegenerate: imageRegenContext?.pokemonInstanceIdToRegenerate,
      pokemonNameToRegenerate: imageRegenContext?.pokemonNameToRegenerate,
    }));

    const fetchOptions = imageRegenContext
      ? { pokemonInstanceIdToRegenerate: imageRegenContext.pokemonInstanceIdToRegenerate, pokemonNameToRegenerate: imageRegenContext.pokemonNameToRegenerate }
      : undefined;

    const handleRetryCallback: OnRetryCallback = (attempt, maxAttempts) => {
      updateGameState(prev => ({
        ...prev,
        aiLoadingStatus: { status: 'retrying_format_error', message: `AI响应格式错误，正在重试 (尝试 ${attempt}/${maxAttempts})...`}
      }));
    };
    ...
    let localAiResponse = { ...aiResponse };
    let newAiSuggestedStartTimeFromThisResponse: number | undefined = undefined;

    if (currentActionIsForCustomizationScreen && playerActionTagOrInput === 'GENERATE_FULL_RANDOM_PROFILE' && localAiResponse.suggestedFullProfileData?.suggestedGameStartTime) {
      newAiSuggestedStartTimeFromThisResponse = localAiResponse.suggestedFullProfileData.suggestedGameStartTime;
    }


    let timeAdvanceOptions: { minutes?: number; hours?: number } = { minutes: Math.floor(Math.random() * 11) + 5 };
    if (localAiResponse.events && localAiResponse.events.some(e => e.type === 'UPDATE_LOCATION')) {
      timeAdvanceOptions = { minutes: Math.floor(Math.random() * 31) + 30 };
    }
    if (playerActionTagOrInput === 'START_ADVENTURE_WITH_CUSTOMIZED_PROFILE') {
      timeAdvanceOptions = { minutes: 0 };
    }


    updateGameState(prev => {
      let newState: GameState = { ...prev, aiLoadingStatus: { status: 'idle' } as LoadingStatus };
      const isNpcChatResponse = playerActionTagOrInput?.startsWith("PLAYER_TALKS_TO_NPC_");

      if (newAiSuggestedStartTimeFromThisResponse !== undefined) {
        newState.aiSuggestedGameStartTime = newAiSuggestedStartTimeFromThisResponse;
      }


      if (playerActionTagOrInput === 'USER_REQUESTS_POKEMON_IMAGE_REGENERATION' && localAiResponse.regeneratedPokemonImageUrl && prev.pokemonInstanceIdToRegenerate) {
        ...
        if (newState.currentBattleEnemy?.instanceId === instanceId) {
             ...
             newState.currentBattleEnemy = { ...newState.currentBattleEnemy, imageUrl: newUrl };
             foundAndUpdated = true;
        }
        if (newState.customizationAssistantResponse?.events) {
            newState.customizationAssistantResponse.events = newState.customizationAssistantResponse.events.map(e => {
                if (e.type === 'PRESENT_SUGGESTED_POKEMON_DETAILS' && e.pokemonDetails?.instanceId === instanceId) {
                    pokemonNameForLog = e.pokemonDetails.name || pokemonNameForLog;
                    foundAndUpdated = true;
                    return {...e, pokemonDetails: {...e.pokemonDetails, imageUrl: newUrl}};
                }
                return e;
            });
        }
        if (foundAndUpdated) {
            const imageUpdateMessage: ChatHistoryEntry = {
                id: `img-regen-${Date.now()}`, timestamp: Date.now(), speaker: "系统",
                narrative: `${pokemonNameForLog} 的图片已尝试更新。`, type: 'system'
            };
            newState.chatHistory = [...newState.chatHistory, imageUpdateMessage];
        }
        newState.currentAIScene = {
            narrative: localAiResponse.narrative || `${pokemonNameForLog} 的图片已尝试更新。`,
            speaker: localAiResponse.speaker || "系统",
            choices: localAiResponse.choices || [{ text: "好的", actionTag: "CONTINUE_AFTER_IMAGE_REGEN" }]
        };
      ...
      if (localAiResponse.events) {
          localAiResponse.events.forEach(event => {
            newState = processAIEvent(newState, event, isNpcChatResponse, currentActionIsForCustomizationScreen ? playerActionTagOrInput as AICustomizationScreenActionTag : undefined);
          });
      }

      if (currentActionIsForCustomizationScreen && playerActionTagOrInput === 'GENERATE_FULL_RANDOM_PROFILE') {
      ...
      if (isNpcChatResponse) {
        ...
        if (newState.currentAIScene && newState.currentAIScene.narrative && newState.currentAIScene.narrative.trim() !== "") {
            const aiSpeaker = newState.currentAIScene.speaker || "旁白";
            const entryType = (aiSpeaker === "系统消息" || aiSpeaker === "系统提示" || aiSpeaker === "系统错误") ? 'system' : 'ai';
            const aiHistoryEntry: ChatHistoryEntry = {
              id: `${Date.now()}-${entryType}-${Math.random().toString(36).substr(2, 9)}`,
              timestamp: Date.now(), speaker: aiSpeaker, narrative: newState.currentAIScene.narrative, type: entryType,
            };
            const lastEntry = newState.chatHistory.length > 0 ? newState.chatHistory[newState.chatHistory.length -1] : null;
            if (!((entryType === 'system' && lastEntry && lastEntry.type === 'system' && lastEntry.narrative === aiHistoryEntry.narrative && (Date.now() - lastEntry.timestamp < 2000)) ||
                  (entryType === 'ai' && lastEntry && lastEntry.type === 'ai' && lastEntry.narrative === aiHistoryEntry.narrative && lastEntry.speaker === aiSpeaker && (Date.now() - lastEntry.timestamp < 1000)))
            ) {
                newState.chatHistory = [...newState.chatHistory, aiHistoryEntry];
            }

            if ((!newState.currentAIScene.choices || newState.currentAIScene.choices.length === 0) &&
                (!newState.currentAIScene.suggestedPlayerReplies || newState.currentAIScene.suggestedPlayerReplies.length === 0) &&
               ...
               newState.currentAIScene.choices = [{ text: "继续...", actionTag: "USER_REQUESTS_CONTINUATION" }];
            }
        }
      }
      newState.pokemonInstanceIdToRegenerate = undefined;
  ...
  const advanceStaticStory = useCallback((segmentId: string) => {
    const newSegment = STORY_DATA[segmentId];
    if (newSegment) {
      setCurrentStaticSegmentId(segmentId);
      updateGameState(prev => {
        let tempState: GameState = { ...prev, currentAIScene: null, customizationAssistantResponse: null, aiLoadingStatus: {status: 'idle'} as LoadingStatus };

        let resolvedSpeaker: string | undefined = typeof newSegment.speaker === 'function' ? newSegment.speaker(tempState.playerProfile) : newSegment.speaker;
        let resolvedText: string = typeof newSegment.text === 'function' ? newSegment.text(tempState.playerProfile, tempState.playerTeam) : newSegment.text;

        if (segmentId === 'CONFIRM_STATIC_BATTLE_SEGMENT' && tempState.pendingBattleDetails) {
            const originalTriggerSegmentId = tempState.pendingBattleDetails.battleReturnSegmentWin === 'BATTLE_WON_DEFAULT'
                ? Object.keys(STORY_DATA).find(key => STORY_DATA[key]?.triggerBattle?.winSegmentId === tempState.pendingBattleDetails?.battleReturnSegmentWin)
                : tempState.pendingBattleDetails.battleReturnSegmentWin.replace('WIN_', 'SEG_');

            const originalSegment = STORY_DATA[originalTriggerSegmentId || prev.currentBattlePlayerPokemonId || ""];
            if(originalSegment) {
                resolvedText = typeof originalSegment.text === 'function' ? originalSegment.text(tempState.playerProfile, tempState.playerTeam) : originalSegment.text;
                resolvedSpeaker = typeof originalSegment.speaker === 'function' ? originalSegment.speaker(tempState.playerProfile) : originalSegment.speaker;
            } else {
        ...
        if (newSegment.onLoad) {
          const tempUpdater = (updaterFn: (prevState: GameState) => GameState) => {
        ...
        const shouldLogStaticText = resolvedText && resolvedText.trim() !== "" && !newSegment.isAIHandoff;
        ...
        if (shouldLogStaticText) {
            ...
            if (!updatedChatHistory.find(entry => entry.narrative === resolvedText && entry.speaker === resolvedSpeaker && entry.type === 'static' && (Date.now() - entry.timestamp < 1000))) {
                updatedChatHistory = [...tempState.chatHistory, staticHistoryEntry];
            }
        }
      ...
      if (newSegment.isAIHandoff) {
        triggerAIStory(newSegment.actionTag || 'CONTINUE_AFTER_STATIC_SEGMENT');
      } else if (!newSegment.choices && newSegment.nextSegmentId && !newSegment.isAIHandoff) {
        setTimeout(() => advanceStaticStory(newSegment.nextSegmentId!), 50);
      }
    } else {
      console.error(`Static story segment "${segmentId}" not found. Attempting AI recovery.`);
      updateGameState(prev => {
        const errorEntry: ChatHistoryEntry = {
          id: `${Date.now()}-error`, timestamp: Date.now(), speaker: "系统错误",
          narrative: `无法找到剧情片段: ${segmentId}。尝试AI恢复。`, type: 'system'
        };
  ...
  const handleStaticStoryChoice = useCallback((choice: StoryChoice) => {
    const playerChoiceNarrative = choice.text;
    updateGameState(prev => ({...prev, chatHistory: [...prev.chatHistory, {id:`${Date.now()}-schoice-${Math.random().toString(36).substr(2, 9)}`, timestamp: Date.now(), speaker: prev.playerProfile.name || "玩家", narrative: playerChoiceNarrative, type:'player_input'}]}),
    ...
    if (choice.isBattleTrigger) {
      ...
      if (currentSegment?.triggerBattle) {
          updateGameState(prev => ({
              ...
              pendingBattleDetails: {
                ...
                battleReturnSegmentWin: currentSegment.triggerBattle!.winSegmentId,
                battleReturnSegmentLose: currentSegment.triggerBattle!.loseSegmentId,
              },
              currentAIScene: null
          }));
      ...
      advanceStaticStory(choice.nextSegmentId);
        ...
        if (choice.isAIHandoff) {
            updateGameState(prev => ({ ...prev, currentAIScene: null, aiLoadingStatus: { status: 'loading', message: 'AI 正在思考...' }, customizationAssistantResponse: null }));
        }
        triggerAIStory(choice.actionTag);
    }
  }, [currentStaticSegmentId, updateGameState, advanceStaticStory, triggerAIStory]);

  const handleAIChoice = useCallback((choice: AIStoryChoice) => {
    const systemAckTags = ["ACKNOWLEDGE_NO_API_KEY", "ACKNOWLEDGE_AI_CONNECTION_ERROR", "ACKNOWLEDGE_AI_DIFFICULTY_PROMPT_NEW_INPUT", "ACKNOWLEDGE_AI_FORMAT_ERROR", "CONTINUE_GENERIC_AI_BLANK"];

    if (choice.actionTag === 'ACKNOWLEDGE_BATTLE_START' || choice.actionTag === 'CONFIRMED_STATIC_BATTLE_FINAL') {
        updateGameState(prev => {
            if (prev.pendingBattleDetails && prev.pendingBattleDetails.enemyPokemon) {
                const playerPokemonForBattle = prev.playerTeam.find(p => !p.isFainted && p.isPlayerOwned);
                if (playerPokemonForBattle && playerPokemonForBattle.instanceId) {
                    return {
                        ...
                        battleReturnSegmentWin: prev.pendingBattleDetails.battleReturnSegmentWin,
                        battleReturnSegmentLose: prev.pendingBattleDetails.battleReturnSegmentLose,
                     ...
                     return {...prev, chatHistory: [...prev.chatHistory, {id:`battle-err-${Date.now()}`,timestamp:Date.now(),speaker:"系统",narrative:noPokemonMsg,type:'system'}], pendingBattleDetails: undefined, currentAIScene: { narrative: noPokemonMsg, choices: [{text: "返回冒险", actionTag: "USER_REQUESTS_CONTINUATION"}]}};
                }
            } else {
                 const noDetailsMsg = choice.actionTag === 'ACKNOWLEDGE_BATTLE_START' ? "(AI未能正确准备战斗数据。)" : "(静态战斗数据准备出错。)";
                 return {...prev, chatHistory: [...prev.chatHistory, {id:`battle-err-${Date.now()}`,timestamp:Date.now(),speaker:"系统错误",narrative: noDetailsMsg, type:'system'}], pendingBattleDetails: undefined, currentAIScene: { narrative: noDetailsMsg, choices: [{text: "返回冒险", actionTag: "USER_REQUESTS_CONTINUATION"}]}};
            }
        });
        return;
    }
    ...
    if (!systemAckTags.includes(choice.actionTag) || choice.actionTag === "USER_REQUESTS_CONTINUATION" || choice.actionTag === "CONTINUE_AFTER_IMAGE_REGEN") {
        updateGameState(prev => ({...prev, chatHistory: [...prev.chatHistory, {id:`${Date.now()}`,timestamp:Date.now(),speaker:prev.playerProfile.name||"玩家",narrative:choice.text,type:'player_input'}]}),
        { minutes: Math.floor(Math.random() * 3) + 1 });
    } else {
        updateGameState(prev => ({...prev, currentAIScene: null, aiLoadingStatus: {status: 'idle'} as LoadingStatus, customizationAssistantResponse: null }));
        if(choice.actionTag === "ACKNOWLEDGE_AI_DIFFICULTY_PROMPT_NEW_INPUT" || choice.actionTag === "ACKNOWLEDGE_AI_FORMAT_ERROR") return;
        return;
    }
  ...
  const handlePlayerCustomInputAction = useCallback((inputText: string) => {
    const trimmedInput = inputText.trim();
    if (trimmedInput) {
      updateGameState(prev => ({...prev, chatHistory: [...prev.chatHistory, {id:`${Date.now()}`,timestamp:Date.now(),speaker:prev.playerProfile.name||"玩家",narrative:trimmedInput,type:'player_input'}]}),
      { minutes: Math.floor(Math.random() * 3) + 1 });
  ...
  const handleBattleEnd = useCallback((
    ...
    let tempNewChatHistory: ChatHistoryEntry[] = [];
    ...
    if (caughtPokemon) {
        ...
        tempNewChatHistory.push({
            id: `catch-success-msg-${Date.now()}`, timestamp: Date.now(), speaker: "系统",
            narrative: `成功捕捉了 ${newlyCaughtPokemon.name}！它已加入你的队伍。`, type: 'system'
        });
        resultSegmentId = STORY_DATA['PLAYER_CAUGHT_POKEMON_SUCCESS'] ? 'PLAYER_CAUGHT_POKEMON_SUCCESS' : 'BATTLE_WON_DEFAULT';

    } else if (didPlayerWin && !usedRun) { // KO Victory
        const baseXP = (finalEnemyPokemonState.level * 25) + 50;
        tempNewChatHistory.push({
            id: `xp-gain-msg-${Date.now()}`, timestamp: Date.now(), speaker: "系统",
            narrative: `战斗胜利！队伍中的宝可梦获得了 ${baseXP} 点经验值！`, type: 'system'
        });
        ...
        processedPlayerTeam = processedPlayerTeam.map(pokemon => {
            if (!pokemon.isFainted && pokemon.isPlayerOwned) {
                ...
                tempNewChatHistory.push({
                    id: `xp-${updatedPokemon.instanceId}-${Date.now()}`, timestamp: Date.now(), speaker: updatedPokemon.name,
                    narrative: `获得了 ${baseXP} 点经验值。`, type: 'system'
                });
                ...
                while (updatedPokemon.currentXp >= updatedPokemon.xpToNextLevel) {
                    ...
                    tempNewChatHistory.push({
                        id: `levelup-${updatedPokemon.instanceId}-${newLevel}-${Date.now()}`, timestamp: Date.now(), speaker: updatedPokemon.name,
                        narrative: `${updatedPokemon.name} 从 ${oldLevel}级 升到了 ${newLevel} 级！`, type: 'system'
                    });
                    ...
                    tempNewChatHistory.push({
                        id: `stats-up-${updatedPokemon.instanceId}-${newLevel}-${Date.now()}`, timestamp: Date.now(), speaker: "系统",
                        narrative: statIncreaseNarrative, type: 'system'
                    });
    ...
    if (STORY_DATA[resultSegmentId]) {
        ...
        updateGameState(prev => ({...prev, chatHistory: [...prev.chatHistory, {id:`${Date.now()}`,timestamp:Date.now(),speaker:"系统警告",narrative:`战斗后剧情片段 "${resultSegmentId}" 未找到。将使用默认后续。`,type:'system'}]}));
  ...
  }, [gameState.battleReturnSegmentWin, gameState.battleReturnSegmentLose, updateGameState, advanceStaticStory, triggerAIStory, sanitizePokemonData]);
  ...
  const handleStartAdventureWithCustomizedProfile = useCallback((customData: CustomizedStartData) => {
    const finalPlayerProfile: PlayerProfile = {
        name: customData.playerProfile.name?.trim() || "冒险者",
        gender: customData.playerProfile.gender || "男", age: customData.playerProfile.age ?? 16,
        description: customData.playerProfile.description?.trim() || "一位普通的冒险者。",
        stamina: customData.playerProfile.stamina ?? INITIAL_GAME_STATE.playerProfile.stamina!,
        maxStamina: customData.playerProfile.maxStamina ?? INITIAL_GAME_STATE.playerProfile.maxStamina!,
        energy: customData.playerProfile.energy ?? INITIAL_GAME_STATE.playerProfile.energy!,
        maxEnergy: customData.playerProfile.maxEnergy ?? INITIAL_GAME_STATE.playerProfile.maxEnergy!,
  ...
  }, [updateGameState, triggerAIStory, sanitizePokemonData, sanitizeItemData, gameState.aiSuggestedGameStartTime]);
  ...
  const handleDirectCustomizationUpdate = useCallback((
    ...
    updateGameState(prev => {
      ...
      switch (field) {
        case 'teamMemberAdd': newPlayerTeam.push(sanitizePokemonData(value as Pokemon)); break;
        case 'teamMemberRemove': newPlayerTeam = newPlayerTeam.filter(p => p.instanceId !== (value as string)); break;
        case 'itemAdd':
          ...
          if (existingItemIndex > -1) newInventory[existingItemIndex].quantity += itemToAdd.quantity;
          else newInventory.push(itemToAdd);
          break;
        case 'itemRemove': newInventory = newInventory.filter(i => i.id !== (value as string)); break;
        case 'itemQtyUpdate':
          const { itemId, quantity } = value as { itemId: string; quantity: number };
          const itemIdxToUpdate = newInventory.findIndex(i => i.id === itemId);
          if (itemIdxToUpdate > -1) {
            if (quantity > 0) newInventory[itemIdxToUpdate].quantity = quantity;
            else newInventory.splice(itemIdxToUpdate, 1);
          }
          break;
        case 'profileFieldUpdate':
          const { field: profileFieldName, value: profileFieldValue } = value as { field: keyof PlayerProfile | 'currentObjective' | 'currentLocationDescription' | 'money', value: any };
          const playerProfileKeys = ['name', 'gender', 'age', 'description', 'stamina', 'maxStamina', 'energy', 'maxEnergy', 'healthStatus'];
          if (playerProfileKeys.includes(profileFieldName)) {
            newPlayerProfile = { ...newPlayerProfile, [profileFieldName]: profileFieldValue };
          ...
          if (typeof profileFieldValue === 'object' && profileFieldValue !== null && 'suggestedGameStartTime' in profileFieldValue && (profileFieldValue as FullProfileSuggestionData).suggestedGameStartTime !== undefined) {
             newAiSuggestedStartTime = (profileFieldValue as FullProfileSuggestionData).suggestedGameStartTime;
             if (newAiSuggestedStartTime) newCurrentGameTime = newAiSuggestedStartTime; // Also update current game time if full profile is accepted
          }
          break;
        case 'gameTimeUpdate':
  ...
  const handleSendCustomizationAssistantMessage = useCallback(async (
    messageText: string,
    ...
    actionTag?: string
      ...
      let specializedSystemPrompt: string = GEMINI_GENERAL_CUSTOMIZATION_CHAT_SYSTEM_PROMPT;
      let specializedUserContentPrompt: string = messageText;

      const handleRetry: OnRetryCallback = (attempt, maxAttempts, errorType) => {
          updateGameState(prev => ({ ...prev, aiLoadingStatus: { status: 'retrying_format_error', message: `助手${errorType === 'format' ? '响应格式' : 'API调用'}错误，重试中 (${attempt}/${maxAttempts})...`}}));
      };

      if (actionTag && actionTag !== 'ASSIST_PROFILE_CUSTOMIZATION') {
          if (actionTag === "ASSIST_PROFILE_CUSTOMIZATION_REQUEST_NEW_POKEMON") {
              classifiedIntent = { intent: "SUGGEST_POKEMON", originalQuery: messageText, params: {} };
          } else if (actionTag === "ASSIST_PROFILE_CUSTOMIZATION_REQUEST_NEW_ITEM") {
              classifiedIntent = { intent: "SUGGEST_ITEM", originalQuery: messageText, params: {} };
          } else if (actionTag === "ASSIST_PROFILE_CUSTOMIZATION_REQUEST_NEW_DESCRIPTION") {
              classifiedIntent = { intent: "REQUEST_RANDOM_DESCRIPTION", originalQuery: messageText, params: {} };
          }
      }

      if (!classifiedIntent) {
          classifiedIntent = await classifyCustomizationIntent(messageText, assistantChatHistoryFromComponent, stateForAIContext, handleRetry);
      }
      ...
      if (classifiedIntent) {
          ...
          switch (classifiedIntent.intent) {
              case "CREATE_FULL_PROFILE":
                  ...
                  if (stateForAIContext.assistantChatJustRefreshed) {
                      ...
                      if (classifiedIntent.params?.theme && classifiedIntent.params.theme.trim() !== "") {
                          themeInstruction = `玩家提供的主题是：“${classifiedIntent.params.theme.trim()}”。请基于此主题生成。`;
                      } else if (classifiedIntent.originalQuery && !["create full profile", "create a new profile", "帮我创建一个档案", "建个档"].includes(classifiedIntent.originalQuery.toLowerCase().trim())) {
                          themeInstruction = `玩家的描述是：“${classifiedIntent.originalQuery}”。请基于此描述生成。`;
                      } else {
                          themeInstruction = "玩家没有提供具体主题或描述，请你完全随机地、创新地生成一个独特且多样化的角色档案。确保所有细节都是全新的。";
                      }
                      contextForSpecializedAgent = `玩家在刷新对话后，要求创建一个全新的角色档案。\n${themeInstruction}\n\n` +
                      ...
                      contextForSpecializedAgent = currentFormStateContext + `玩家要求基于以下主题或描述创建一个完整的角色档案：“${classifiedIntent.params?.theme || classifiedIntent.originalQuery}”。请严格遵循 \`GEMINI_FULL_PROFILE_CREATOR_SYSTEM_PROMPT\` 的指示生成JSON响应，包括一个合适的 \`suggestedGameStartTime\` (Unix 毫秒时间戳)。`;
                  }
                  break;
              case "SUGGEST_POKEMON":
                  specializedSystemPrompt = GEMINI_STARTER_POKEMON_SUGGESTOR_SYSTEM_PROMPT;
                  let pokemonQuery = `玩家想要一个初始宝可梦建议。原始请求：“${classifiedIntent.originalQuery}”。`;
                  if (classifiedIntent.params?.pokemonType) pokemonQuery += ` 特别指定属性：“${classifiedIntent.params.pokemonType}”。`;
                  if (classifiedIntent.params?.pokemonName) pokemonQuery += ` 提及名称：“${classifiedIntent.params.pokemonName}”。`;
                  pokemonQuery += ` 请严格遵循 \`GEMINI_STARTER_POKEMON_SUGGESTOR_SYSTEM_PROMPT\` 的指示生成JSON响应。`;
                  contextForSpecializedAgent = pokemonQuery;
                  break;
              case "SUGGEST_ITEM":
                  specializedSystemPrompt = GEMINI_STARTER_ITEM_SUGGESTOR_SYSTEM_PROMPT;
                  let itemQuery = `玩家想要一个初始道具建议。原始请求：“${classifiedIntent.originalQuery}”。`;
                  if (classifiedIntent.params?.itemName) itemQuery += ` 提及道具名：“${classifiedIntent.params.itemName}”。`;
                  if (classifiedIntent.params?.quantity) itemQuery += ` 请求数量：${classifiedIntent.params.quantity}。`;
              ...
              case "MODIFY_PROFILE_FIELD":
                  specializedSystemPrompt = GEMINI_PROFILE_FIELD_ADVISOR_SYSTEM_PROMPT;
              ...
              case "MODIFY_OBJECTIVE":
                  specializedSystemPrompt = GEMINI_OBJECTIVE_LOCATION_MONEY_ADVISOR_SYSTEM_PROMPT;
              ...
              case "SUGGEST_GAME_START_TIME_BASED_ON_PROFILE": // Should be handled by requestDynamicTimeSuggestion now
                  specializedSystemPrompt = GEMINI_DYNAMIC_GAME_TIME_SUGGESTOR_SYSTEM_PROMPT;
              ...
              case "REQUEST_RANDOM_DESCRIPTION":
                  specializedSystemPrompt = GEMINI_RANDOM_DESCRIPTION_GENERATOR_ASSISTANT_PROMPT;
              ...
              case "UNKNOWN_INTENT":
              default:
                  specializedSystemPrompt = GEMINI_GENERAL_CUSTOMIZATION_CHAT_SYSTEM_PROMPT;
                  let generalQuery = `玩家的意图是“${classifiedIntent.intent}”。原始请求：“${classifiedIntent.originalQuery}”。`;
                  if (classifiedIntent.feedbackToUser) generalQuery += ` 分类器反馈: "${classifiedIntent.feedbackToUser}"`;
          ...
          if (!(stateForAIContext.assistantChatJustRefreshed && classifiedIntent?.intent === "CREATE_FULL_PROFILE")) {
              let currentFormStateContext = `当前自定义状态概要：\n`;
              ...
              let formattedYearString = jsYear <= 0 ? `公元前 ${Math.abs(jsYear) +1}` : `公元 ${jsYear}`;
              ...
              contextForSpecializedAgent = currentFormStateContext + `玩家的最新请求/关注点：\n${contextForSpecializedAgent}\n\n`;
          }
      ...
      updateGameState(prev => ({ ...prev, aiLoadingStatus: { status: 'loading', message: '助手思考中...' }}));
      ...
      if (classifiedIntent?.intent === "CREATE_FULL_PROFILE" && aiResponse.suggestedFullProfileData?.suggestedGameStartTime) {
        newAiSuggestedStartTime = aiResponse.suggestedFullProfileData.suggestedGameStartTime;
      }
      ...
      updateGameState(prev => ({
        ...
        aiSuggestedGameStartTime: newAiSuggestedStartTime ?? prev.aiSuggestedGameStartTime
      }));
  }, [gameState, updateGameState, classifyCustomizationIntent]);


  const requestDynamicTimeSuggestion = useCallback(async (profileData: ProfileDataForTimeSuggestion) => {
    const stateForAIContext = { ...gameState };
    updateGameState(prev => ({
        ...prev,
    ...
    const handleRetry: OnRetryCallback = (attempt, maxAttempts, errorType) => {
        updateGameState(prev => ({ ...prev, aiLoadingStatus: { status: 'retrying_format_error', message: `助手响应格式错误，建议时间重试中 (${attempt}/${maxAttempts})...`}}));
    };
  ...
  const handleSavePlayerProfileChanges = useCallback((updatedProfile: PlayerProfile) => {
    updateGameState(prev => ({ ...prev, playerProfile: { ...prev.playerProfile, ...updatedProfile } }), { minutes: 1 });
     const profileUpdateMessage: ChatHistoryEntry = {
        id: `profile-update-${Date.now()}`, timestamp: Date.now(), speaker: "系统", narrative: "你的角色信息已更新。", type: 'system',
      };
      updateGameState(prev => ({ ...prev, chatHistory: [...prev.chatHistory, profileUpdateMessage] }));
  }, [updateGameState]);

  const handleRegeneratePokemonImage = useCallback((instanceId: string) => {
    const pokemonToRegen =
        gameState.playerTeam.find(p => p.instanceId === instanceId) ||
        (gameState.currentBattleEnemy?.instanceId === instanceId ? gameState.currentBattleEnemy : undefined) ||
        (gameState.customizationAssistantResponse?.events?.find(e => e.type === 'PRESENT_SUGGESTED_POKEMON_DETAILS' && e.pokemonDetails?.instanceId === instanceId)?.pokemonDetails as Pokemon | undefined);
    if (pokemonToRegen) {
        triggerAIStory('USER_REQUESTS_POKEMON_IMAGE_REGENERATION', false, {
            pokemonInstanceIdToRegenerate: instanceId, pokemonNameToRegenerate: pokemonToRegen.name,
        });
    } else {
        console.warn("Could not find Pokémon with instanceId for image regeneration:", instanceId);
         updateGameState(prev => ({ ...prev, chatHistory: [...prev.chatHistory, {id: `err-img-regen-${Date.now()}`, timestamp: Date.now(), speaker: "系统错误", narrative: `无法为ID ${instanceId} 的宝可梦重新生成图片，未找到该宝可梦。`, type: 'system'}] }));
    }
  }, [gameState, triggerAIStory, updateGameState]);

  useEffect(() => {
    if (gameState.gameMode === GameMode.CUSTOMIZE_RANDOM_START && !gameState.initialProfileGenerated && gameState.aiLoadingStatus.status === 'idle') {
      triggerAIStory('GENERATE_FULL_RANDOM_PROFILE', true);
    }
  }, [gameState.gameMode, gameState.initialProfileGenerated, gameState.aiLoadingStatus.status, triggerAIStory]);

  const [npcInteractionLoading, setNpcInteractionLoading] = useState(false);

  const fetchInitialNPCDialogueAndOrSuggestions = useCallback(async (npcToChatWith: NPC, setSelectedNPCForChat: Function, setNpcChatSuggestions: Function) => {
    setNpcInteractionLoading(true);
    updateGameState(prev => ({ ...prev, aiLoadingStatus: { status: 'loading', message: '获取NPC对话...' } }));

    let npcSystemPrompt = GEMINI_NPC_CHAT_SYSTEM_PROMPT;
    ...
    const handleRetry: OnRetryCallback = (attempt, maxAttempts) => {
        updateGameState(prev => ({ ...prev, aiLoadingStatus: { status: 'retrying_format_error', message: `AI响应格式错误，获取NPC对话重试中 (${attempt}/${maxAttempts})...`}}));
    };

    const aiResponse = await fetchStoryContinuation(
        { ...gameState, knownNPCs: gameState.knownNPCs.map(n => ({ ...n, dialogueHistory: [] })) } as GameState,
    ...
    const setNpcChatHistoryAndSelectNpc = (npcToUpdate: NPC, initialNarrative?: string, suggestions?: AIStoryChoice[]) => {
        updateGameState(prev => {
            ...
            const updatedNpcs = prev.knownNPCs.map(npc =>
                npc.id === npcToUpdate.id ? { ...npc, dialogueHistory: newDialogueHistory } : npc
            );
            return { ...prev, knownNPCs: updatedNpcs };
        });
        setSelectedNPCForChat((prevNpc: NPC | null) => prevNpc ? { ...prevNpc, dialogueHistory: initialNarrative ? [{id: `sel-npc-initial-${prevNpc.id}-${Date.now()}`, timestamp: Date.now(), speaker: prevNpc.name, narrative: initialNarrative, type: 'npc_dialogue'}] : [] } : null);
        setNpcChatSuggestions(suggestions || null);
    };

    setNpcChatHistoryAndSelectNpc(npcToChatWith, aiResponse.narrative, aiResponse.suggestedPlayerReplies);
    updateGameState(prev => ({ ...prev, aiLoadingStatus: { status: 'idle' } as LoadingStatus}));
    setNpcInteractionLoading(false);
  }, [gameState, updateGameState, processAIEvent]);

  const handleSendPlayerMessageToNPC = useCallback(async (npcId: string, messageText: string, currentNpcContext: NPC, setSelectedNPCForChat: Function, setNpcChatSuggestions: Function, suggestionActionTag?: string) => {
    setNpcInteractionLoading(true);
    updateGameState(prev => ({ ...prev, aiLoadingStatus: { status: 'loading', message: 'NPC回应中...' }}));

    const playerEntry: ChatHistoryEntry = {
        id: `player-msg-${npcId}-${Date.now()}`, timestamp: Date.now(), speaker: gameState.playerProfile.name || "你",
        narrative: messageText, type: 'player_input'
    };
    ...
    updateGameState(prev => {
        const updatedNpcs = prev.knownNPCs.map(npc =>
            npc.id === npcId ? { ...npc, dialogueHistory: [...npc.dialogueHistory, playerEntry] } : npc
        );
        return { ...prev, knownNPCs: updatedNpcs };
    });
    ...
    const handleRetry: OnRetryCallback = (attempt, maxAttempts) => {
      updateGameState(prev => ({ ...prev, aiLoadingStatus: { status: 'retrying_format_error', message: `AI响应格式错误，NPC回应重试中 (${attempt}/${maxAttempts})...`}}));
    };

    const aiResponse = await fetchStoryContinuation(
        { ...gameState, knownNPCs: gameState.knownNPCs.map(n => ({ ...n, dialogueHistory: n.id === npcId ? recentHistory : [] })) } as GameState,
        npcChatPrompt,
        undefined,
        handleRetry,
        0,
    ...
    const npcResponseEntry: ChatHistoryEntry | null = aiResponse.narrative
      ? {
          id: `npc-reply-${npcId}-${Date.now()}`, timestamp: Date.now(), speaker: currentNpcContext.name,
          narrative: aiResponse.narrative, type: 'npc_dialogue'
        }
      : null;
    ...
    if (npcResponseEntry) {
        const finalNpcContext = {
            ...
            dialogueHistory: [...updatedCurrentNpcContext.dialogueHistory, npcResponseEntry]
        };
        setSelectedNPCForChat(finalNpcContext);
        updateGameState(prev => {
            const updatedNpcs = prev.knownNPCs.map(npc =>
                npc.id === npcId ? { ...npc, dialogueHistory: [...npc.dialogueHistory, npcResponseEntry] } : npc
            );
            return { ...prev, knownNPCs: updatedNpcs };
        });
    }
