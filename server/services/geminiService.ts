...
import { GameState, AIStoryResponse, ChatHistoryEntry, NPC, AIStoryChoice, AIEventTrigger, PlayerProfile, AICustomizationScreenActionTag, Pokemon, InventoryItem, PokemonMoveInstance, ParsedBattleAction, BattleCommandParseContext, PokemonType, ClassifiedIntent, CustomizationIntentType, ActiveStatusCondition } from '../types'; // Added ClassifiedIntent, CustomizationIntentType, ActiveStatusCondition
import { 
...
export type OnRetryCallback = (attempt: number, maxAttempts: number, errorType: 'format' | 'api') => void;


function buildPrompt(gameState: GameState, lastPlayerActionTagOrInput?: string, options?: FetchOptions): string {
  let prompt = "";
  const customizationScreenActionTags: AICustomizationScreenActionTag[] = ['GENERATE_FULL_RANDOM_PROFILE', 'GENERATE_RANDOM_STARTER_POKEMON', 'GENERATE_RANDOM_ITEM', 'GENERATE_RANDOM_PLAYER_DESCRIPTION'];
  const isStrictActionForThisPromptBuilder = customizationScreenActionTags.includes(lastPlayerActionTagOrInput as AICustomizationScreenActionTag) ||
                                           lastPlayerActionTagOrInput === 'USER_REQUESTS_POKEMON_IMAGE_REGENERATION';


  if (lastPlayerActionTagOrInput === 'USER_REQUESTS_POKEMON_IMAGE_REGENERATION' && options?.pokemonInstanceIdToRegenerate && options?.pokemonNameToRegenerate) {
  ...
  } else if (customizationScreenActionTags.includes(lastPlayerActionTagOrInput as AICustomizationScreenActionTag)) {
    if (lastPlayerActionTagOrInput === 'GENERATE_FULL_RANDOM_PROFILE') {
      prompt = "Player requests a full random starting profile for a Pokémon text adventure.\n";
    ...
    } else if (lastPlayerActionTagOrInput === 'GENERATE_RANDOM_PLAYER_DESCRIPTION') {
      prompt = "Player (on main customization form) requests a new random character description.\n";
      prompt += "Instruction: You are operating under the 'GENERATE_RANDOM_PLAYER_DESCRIPTION' action tag. Provide a 'SET_PLAYER_PROFILE' event with only a new 'description' (Chinese) in 'profileDetails'. The narrative should confirm generation and suggest editing or acceptance, all in Chinese.\n";
    } else if (lastPlayerActionTagOrInput === 'GENERATE_RANDOM_STARTER_POKEMON') {
      const currentStartersString = gameState.playerTeam.length > 0 ? gameState.playerTeam.map(p => p.name).join(', ') : "无";
      ...
      prompt = "Player (on main customization form) requests a new random item to be added.\n";
    ...
    prompt = "Current Game State (all user-facing text like names, descriptions should be considered as Chinese):\n";
    const profileToUse = gameState.playerProfile || INITIAL_GAME_STATE.playerProfile;
    
    const gameTime = new Date(gameState.currentGameTime);
    const jsYear = gameTime.getFullYear();
    let formattedYearString;
    if (jsYear === 0) { // JavaScript year 0 corresponds to 1 BC
        formattedYearString = "公元前 1年";
    } else if (jsYear < 0) { // JavaScript year -N corresponds to (N+1) BC
        formattedYearString = `公元前 ${Math.abs(jsYear) + 1}年`;
    } else { // Positive years are AD/CE
    ...
    if (lastPlayerActionTagOrInput && !lastPlayerActionTagOrInput.startsWith("PLAYER_TALKS_TO_NPC_") && !customizationScreenActionTags.includes(lastPlayerActionTagOrInput as AICustomizationScreenActionTag)) {
      prompt += `\nPlayer's Last Action/Choice Tag or Input (Chinese text if applicable): ${lastPlayerActionTagOrInput}\n`;
      if (lastPlayerActionTagOrInput === 'START_ADVENTURE_WITH_CUSTOMIZED_PROFILE') {
    ...
    } else if (!gameState.chatHistory.some(entry => entry.type === 'ai') && !lastPlayerActionTagOrInput?.startsWith("PLAYER_TALKS_TO_NPC_") && !customizationScreenActionTags.includes(lastPlayerActionTagOrInput as AICustomizationScreenActionTag)) {
...
export async function fetchStoryContinuation(
  ...
  if (!API_KEY) {
    return {
      narrative: "游戏遇到了一点小问题，AI暂时无法连接。请检查API密钥设置或稍后再试。",
      speaker: "系统消息",
  ...
  const isSpecializedAssistantCall = 
      ...
      systemPromptOverride !== GEMINI_BATTLE_ITEM_ACTION_SUGGESTOR_SYSTEM_PROMPT && // Added check
  ...
  if (isSpecializedAssistantCall) {
    ...
    if (systemPromptOverride === GEMINI_GAME_MASTER_SYSTEM_PROMPT || systemPromptOverride === GEMINI_STRICT_ACTION_SYSTEM_PROMPT) {
        userPromptContent = buildPrompt(gameState, playerActionTagOrInputOrSpecializedPrompt, options);
    } else {
        userPromptContent = playerActionTagOrInputOrSpecializedPrompt;
    }
  }
  ...
  try {
    const response: GenerateContentResponse = await ai.models.generateContent({
      ...
      config: {
        ...
        temperature: (systemPromptOverride === GEMINI_CUSTOMIZATION_INTENT_CLASSIFIER_SYSTEM_PROMPT || systemPromptOverride === GEMINI_BATTLE_COMMAND_PARSER_SYSTEM_PROMPT || systemPromptOverride === GEMINI_BATTLE_ITEM_ACTION_SUGGESTOR_SYSTEM_PROMPT) ? 0.1 : 0.75, 
        topK: (systemPromptOverride === GEMINI_CUSTOMIZATION_INTENT_CLASSIFIER_SYSTEM_PROMPT || systemPromptOverride === GEMINI_BATTLE_COMMAND_PARSER_SYSTEM_PROMPT || systemPromptOverride === GEMINI_BATTLE_ITEM_ACTION_SUGGESTOR_SYSTEM_PROMPT) ? 20 : 40,
        topP: 0.95,
        thinkingConfig: (systemPromptOverride === GEMINI_CUSTOMIZATION_INTENT_CLASSIFIER_SYSTEM_PROMPT || systemPromptOverride === GEMINI_BATTLE_COMMAND_PARSER_SYSTEM_PROMPT || systemPromptOverride === GEMINI_BATTLE_ITEM_ACTION_SUGGESTOR_SYSTEM_PROMPT || systemPromptOverride === GEMINI_MOVE_DESCRIPTION_SYSTEM_PROMPT) ? {thinkingBudget: 0} : undefined,
      }
    });
    ...
    try {
      ...
      const typedEvents = Array.isArray(parsedData.events)
        ? parsedData.events.map((event: any) => {
            if (event.type === 'SET_PLAYER_PROFILE' && event.profileDetails) {
              return { ...event, profileDetails: event.profileDetails as Partial<PlayerProfile> };
            }
            if ((event.type === 'ADD_POKEMON_TO_TEAM' || event.type === 'PRESENT_SUGGESTED_POKEMON_DETAILS') && event.pokemonDetails) {
                const aiProvidedPokemonData = event.pokemonDetails;
                let sanitizedMovesArray: PokemonMoveInstance[] | undefined = undefined;

                if (aiProvidedPokemonData.moves && Array.isArray(aiProvidedPokemonData.moves)) {
                    sanitizedMovesArray = aiProvidedPokemonData.moves.map((move: any): PokemonMoveInstance => {
                        let category: '物理' | '特殊' | '变化';
                        if (move.category && (move.category === '物理' || move.category === '特殊' || move.category === '变化')) {
                        ...
                        const basePP = typeof move.basePP === 'number' && move.basePP >= 0 ? move.basePP : 10;

                        let moveType = PokemonType.NORMAL;
                        if (move.type && Object.values(PokemonType).includes(move.type as PokemonType)) {
                            moveType = move.type as PokemonType;
                        } else if (typeof move.type === 'string') {
                            const matchedKey = Object.keys(PokemonType).find(key => PokemonType[key as keyof typeof PokemonType] === move.type);
                            if (matchedKey) {
                                moveType = PokemonType[matchedKey as keyof typeof PokemonType];
                            }
                        }
                        ...
                        return {
                            ...
                            description: typeof move.description === 'string' ? move.description : undefined, 
                            effects: Array.isArray(move.effects) ? move.effects.map(eff => ({...eff, effectString: eff.effectString || "该效果没有额外说明"})) : undefined, 
                        };
                    });
                }
            ...
            if (event.type === 'PRESENT_SUGGESTED_ITEM_DETAILS' && event.itemDetails) {
                return { ...event, itemDetails: {...(event.itemDetails as Partial<InventoryItem>), name: event.itemDetails.name || "未知道具", effectText: event.itemDetails.effectText || "效果未知", description: event.itemDetails.description || "一个神秘的道具"}, quantity: event.quantity as number|undefined };
            }
            return event;
      ...
      if (Array.isArray(parsedData.choices)) {
        const filteredChoices = parsedData.choices.filter((c: any) => c && typeof c.text === 'string' && typeof c.actionTag === 'string');
        if (filteredChoices.length > 0) {
          finalChoices = filteredChoices.map(c => ({...c, text: c.text || "继续"})) as AIStoryChoice[]; 
        }
      }

      let finalSuggestedPlayerReplies: AIStoryChoice[] | undefined = undefined;
      if (Array.isArray(parsedData.suggestedPlayerReplies)) {
        const filteredReplies = parsedData.suggestedPlayerReplies.filter((r: any) => r && typeof r.text === 'string' && typeof r.actionTag === 'string');
        if (filteredReplies.length > 0) {
          finalSuggestedPlayerReplies = filteredReplies.map(r => ({...r, text: r.text || "好的"})) as AIStoryChoice[];
        }
      }

      const aiResponse: AIStoryResponse = {
        narrative: typeof parsedData.narrative === 'string' ? parsedData.narrative : "AI正在思考...", 
        speaker: typeof parsedData.speaker === 'string' ? parsedData.speaker : "旁白", 
        imageUrl: typeof parsedData.imageUrl === 'string' ? parsedData.imageUrl : undefined,
        ...
        regeneratedPokemonImageUrl: typeof parsedData.regeneratedPokemonImageUrl === 'string' ? parsedData.regeneratedPokemonImageUrl : undefined,
        suggestedFullProfileData: parsedData.suggestedFullProfileData, 
      };

      const customizationScreenActionTags: AICustomizationScreenActionTag[] = ['GENERATE_FULL_RANDOM_PROFILE', 'GENERATE_RANDOM_STARTER_POKEMON', 'GENERATE_RANDOM_ITEM', 'GENERATE_RANDOM_PLAYER_DESCRIPTION'];
      const isStrictCustomizationScreenResponse = customizationScreenActionTags.includes(playerActionTagOrInputOrSpecializedPrompt as AICustomizationScreenActionTag) && systemPromptOverride === GEMINI_STRICT_ACTION_SYSTEM_PROMPT;
      const isNpcChatResponse = systemPromptOverride === GEMINI_NPC_CHAT_SYSTEM_PROMPT;
      const isImageRegenResponse = playerActionTagOrInputOrSpecializedPrompt === 'USER_REQUESTS_POKEMON_IMAGE_REGENERATION' && systemPromptOverride === GEMINI_STRICT_ACTION_SYSTEM_PROMPT;
      const isCustomizationAssistantCall = isSpecializedAssistantCall; 
      const isBattleItemSuggesterCall = systemPromptOverride === GEMINI_BATTLE_ITEM_ACTION_SUGGESTOR_SYSTEM_PROMPT;
      ...
      if (isBattleItemSuggesterCall) {
        ...
        if (!aiResponse.narrative) aiResponse.narrative = "你想如何使用此道具？";
        if (!aiResponse.itemActionSuggestions) aiResponse.itemActionSuggestions = [];
        ...
        aiResponse.choices = parsedData.choices || [{ text: "好的", actionTag: "CONTINUE_AFTER_IMAGE_REGEN" }];
        aiResponse.events = undefined;
      } else if (isStrictCustomizationScreenResponse) {
        if (!aiResponse.narrative) { 
            if(playerActionTagOrInputOrSpecializedPrompt === 'GENERATE_FULL_RANDOM_PROFILE') aiResponse.narrative = "已为你生成初始身份信息。你可以在下方调整，或直接开始冒险！";
            else if (playerActionTagOrInputOrSpecializedPrompt === 'GENERATE_RANDOM_PLAYER_DESCRIPTION') aiResponse.narrative = "新的人物说明已生成。"; 
            else if (playerActionTagOrInputOrSpecializedPrompt === 'GENERATE_RANDOM_STARTER_POKEMON') aiResponse.narrative = "为你随机添加了新的初始宝可梦选项...";
            else if (playerActionTagOrInputOrSpecializedPrompt === 'GENERATE_RANDOM_ITEM') aiResponse.narrative = "为你随机添加了新的道具选项...";
        ...
        if (!aiResponse.narrative && (!aiResponse.events || aiResponse.events.length === 0) && (!aiResponse.choices || aiResponse.choices.length ===0) && (!aiResponse.suggestedPlayerReplies || aiResponse.suggestedPlayerReplies.length === 0) && !aiResponse.suggestedFullProfileData) {
            aiResponse.narrative = "我不太明白你的意思，可以换个说法吗？";
        } else if (!aiResponse.narrative && ( (aiResponse.events && aiResponse.events.length > 0) || aiResponse.suggestedFullProfileData) ) {
            aiResponse.narrative = "请看这个建议：";
        }
      } else if (isNpcChatResponse) {
        const npcIdFromTagParts = playerActionTagOrInputOrSpecializedPrompt.split('_');
        const npcIdFromTag = npcIdFromTagParts.length > 3 ? npcIdFromTagParts[3] : "unknown_npc";
        ...
        if (playerActionTagOrInputOrSpecializedPrompt.endsWith("_OPEN_CHAT")) {
            if (aiResponse.narrative && aiResponse.narrative.trim() !== "" && aiResponse.narrative !== "..." && !aiResponse.narrative.toLowerCase().includes(npcName.toLowerCase())) {
            } else {
                 aiResponse.narrative = ""; 
            }
            if (!aiResponse.suggestedPlayerReplies || aiResponse.suggestedPlayerReplies.length === 0) {
                aiResponse.suggestedPlayerReplies = [
                    { text: "你好！", actionTag: `PLAYER_TALKS_TO_NPC_${npcIdFromTag}_MESSAGE:你好！` },
                    { text: "在吗？", actionTag: `PLAYER_TALKS_TO_NPC_${npcIdFromTag}_MESSAGE:在吗？` }
                ];
            }
        } else if (aiResponse.narrative && aiResponse.narrative.trim() !== "") {
            if (!aiResponse.suggestedPlayerReplies || aiResponse.suggestedPlayerReplies.length === 0) {
                 aiResponse.suggestedPlayerReplies = [{ text: "嗯。", actionTag: `PLAYER_TALKS_TO_NPC_${npcIdFromTag}_ACKNOWLEDGE` }];
            }
        } else if (!aiResponse.narrative && aiResponse.suggestedPlayerReplies && aiResponse.suggestedPlayerReplies.length > 0) {
        } else { 
            aiResponse.narrative = `(${aiResponse.speaker || npcName} 似乎无话可说。)`;
            aiResponse.suggestedPlayerReplies = [{ text: "继续...", actionTag: `PLAYER_TALKS_TO_NPC_${npcIdFromTag}_REQUEST_CONTINUATION` }];
        }
      } else { 
        ...
        if (aiResponse.narrative && aiResponse.narrative.trim() !== "") {
            if (!aiResponse.choices || aiResponse.choices.length === 0) {
                const systemMessageKeywords = ["API密钥", "连接AI时", "AI似乎遇到了理解上的困难", "AI响应的格式似乎有些问题"];
                const isLikelySystemErrorNarrative = systemMessageKeywords.some(keyword => aiResponse.narrative.includes(keyword));
                if (!isLikelySystemErrorNarrative) { 
                     aiResponse.choices = [{ text: "继续...", actionTag: "USER_REQUESTS_CONTINUATION" }];
                }
            }
        }
        else if ((!aiResponse.narrative || aiResponse.narrative.trim() === "") && ((aiResponse.choices && aiResponse.choices.length > 0) || (aiResponse.events && aiResponse.events.length > 0))) {
            ...
            aiResponse.speaker = aiResponse.speaker || "旁白";
            aiResponse.choices = [{ text: "尝试继续...", actionTag: "USER_REQUESTS_CONTINUATION" }];
        }
      }
      return aiResponse;
    } catch (parseError) {
      ...
      if (jsonText !== rawTrimmedText) console.error("Original raw text from AI (before fence removal):", rawTrimmedText);

      if (retryCount < MAX_RETRIES) {
        if (onRetry) {
          onRetry(retryCount + 1, MAX_RETRIES + 1, 'format');
        }
        await new Promise(resolve => setTimeout(resolve, 750 + retryCount * 250));
        return fetchStoryContinuation(gameState, playerActionTagOrInputOrSpecializedPrompt, options, onRetry, retryCount + 1, systemPromptOverride);
      } else {
        return {
          narrative: "AI响应的格式似乎有些问题，多次尝试后仍无法正确显示。请尝试重新操作或简化您的请求。",
          speaker: "系统消息",
          choices: [{ text: "知道了", actionTag: "ACKNOWLEDGE_AI_FORMAT_ERROR" }],
        };
      }
    }
  } catch (apiError) {
    ...
    if (apiError instanceof Error) errorMessage += ` 错误信息: ${apiError.message}`;
    return {
      narrative: errorMessage, speaker: "系统消息",
...
export async function classifyCustomizationIntent(
    ...
    if (!API_KEY) {
        ...
        return { intent: "UNKNOWN_INTENT", originalQuery: userInput, feedbackToUser: "AI连接中断，无法解析您的请求。" };
    }
    ...
    try {
        ...
        if (!parsed.intent) { // Basic validation
            throw new Error("Intent classifier returned JSON without an 'intent' field.");
        }
        ...
        if (retryCount < MAX_RETRIES) {
            ...
            return classifyCustomizationIntent(userInput, currentChatHistory, gameStateSnapshot, onRetry, retryCount + 1);
        }
        return { intent: "UNKNOWN_INTENT", originalQuery: userInput, feedbackToUser: "解析您的请求时遇到问题，请尝试换一种说法。" };
    }
}


export async function fetchMoveDescription(moveName: string, pokemonName?: string): Promise<string> {
  if (!API_KEY) {
  ...
  userPrompt += "描述应简洁（例如，少于60字），适合游戏内显示。确保描述是中文。";
...
export async function parsePlayerBattleCommand(
  ...
  const switchableTeam = context.playerTeam.filter(p => p.instanceId !== context.activePlayerPokemon?.instanceId && !p.isFainted);
  ...
  try {
    ...
    if (parsedData.feedbackMessage && !/[\u4e00-\u9fa5]/.test(parsedData.feedbackMessage)) { 
        console.warn("AI feedbackMessage might not be in Chinese:", parsedData.feedbackMessage);
        parsedData.feedbackMessage = "处理指令时发生未知错误。"; 
    }
    if (parsedData.actionType === "NOT_ENOUGH_PP" && context.activePlayerPokemon && parsedData.moveName) {
        const moveInstance = context.activePlayerPokemon.moves.find(m => m.name === parsedData.moveName);
        if (moveInstance && moveInstance.currentPP > 0) {
            console.warn(`AI reported NOT_ENOUGH_PP for ${parsedData.moveName} but it has ${moveInstance.currentPP}/${moveInstance.basePP} PP. Overriding to USE_MOVE.`);
    ...
    if (retryCount < MAX_RETRIES) {
      ...
      await new Promise(resolve => setTimeout(resolve, 500 + retryCount * 250));
      return parsePlayerBattleCommand(commandText, context, onRetry, retryCount + 1);
    }
    return {
      actionType: "UNKNOWN",
      feedbackMessage: "AI对战斗指令的响应格式似乎有问题，多次尝试后仍无法解析。请重试或使用按钮操作。"
    };
  }
}
...
export async function fetchBattleItemActionSuggestions(
  ...
  if (!API_KEY) {
    return { narrative: "AI连接中断，无法获取道具建议。", itemActionSuggestions: [] };
  }
  ...
  const simplifiedActivePlayerPokemon = {
    ...
    statusConditions: activePlayerPokemon.statusConditions.map(sc => ({condition: sc.condition})) as ActiveStatusCondition[] // Only send condition
  };
  const simplifiedEnemyPokemon = {
    ...
    statusConditions: enemyPokemon.statusConditions.map(sc => ({condition: sc.condition})) as ActiveStatusCondition[]
  };
  const simplifiedBenchedPlayerPokemon = benchedPlayerPokemon.map(p => ({
    ...
    maxHp: p.maxHp,
    statusConditions: p.statusConditions.map(sc => ({condition: sc.condition})) as ActiveStatusCondition[]
  }));
...
Please provide item action suggestions based on this context.
  ...
  try {
    ...
    if (!parsedData.narrative || !Array.isArray(parsedData.itemActionSuggestions)) {
      throw new Error("AI response for item suggestions is missing narrative or itemActionSuggestions array.");
    }
    parsedData.itemActionSuggestions = parsedData.itemActionSuggestions.filter(
      (s: AIStoryChoice) => s && typeof s.text === 'string' && typeof s.actionTag === 'string'
    );
    ...
    if (retryCount < MAX_RETRIES) {
      ...
      await new Promise(resolve => setTimeout(resolve, 500 + retryCount * 250));
      return fetchBattleItemActionSuggestions(itemName, activePlayerPokemon, enemyPokemon, benchedPlayerPokemon, onRetry, retryCount + 1);
    }
    return {
