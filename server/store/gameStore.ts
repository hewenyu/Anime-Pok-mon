...
export const useGameStore = create<GameStore>((set) => ({
  ...
  advanceGameTime: (options: { minutes?: number; hours?: number }) => set((state) => {
  ...
  setAILoadingStatus: (status: LoadingStatus) => set({ aiLoadingStatus: status }),
  
  addBattleToHistory: (battle: any) => set((state) => ({
    battleHistory: [...state.battleHistory, battle]
  })),
  
  setHasAttemptedInitialLoad: (attempted: boolean) => set({ hasAttemptedInitialLoad: attempted }),
