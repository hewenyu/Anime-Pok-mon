...
const PlayerStatBar: React.FC<{label: string, current: number, max: number, barColorClass: string, textColorClass?: string}> =
    ({label, current, max, barColorClass, textColorClass = "text-gray-700"}) => {
    ...
    return (
        <div className="my-1.5">
            <div className="flex justify-between items-baseline text-sm mb-0.5">
                ...
                <span className={`text-xs ${textColorClass}`}>{current} / {max}</span>
            </div>
...
const SectionTitle: React.FC<{children: React.ReactNode, className?: string}> = ({children, className}) => (
  <h3 className={`text-lg font-semibold mb-2 text-purple-600 border-t border-purple-300/70 pt-3 ${className || ''}`}>{children}</h3>
);
    ...
    <div className="sidebar-panel h-full overflow-y-auto custom-scrollbar flex flex-col">
      ...
      <div className="mb-3">
        <h2
          ...
          role="button"
          tabIndex={0}
          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onEditPlayerProfile();}}
          title="编辑角色信息"
        ...
        <div className="text-sm space-y-0.5">
          <p><span className="font-semibold text-sky-700">性别:</span> <span className="text-gray-800">{playerProfile.gender}</span></p>
          {playerProfile.age && <p><span className="font-semibold text-sky-700">年龄:</span> <span className="text-gray-800">{playerProfile.age}</span></p>}
          {playerProfile.description && <p><span className="font-semibold text-sky-700">说明:</span> <span className="text-gray-800 italic text-xs">{playerProfile.description}</span></p>}
          ...
          />
          <p className="mt-1.5"><span className="font-semibold text-sky-700">健康状况:</span> <span className="text-gray-800">{playerProfile.healthStatus}</span></p>
          <div className="mt-2 pt-2 border-t border-purple-300/50">
            <span className="font-semibold text-sky-700">金钱: </span>
            <span className="text-lg text-green-600 font-mono">${money.toLocaleString()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mb-3">
        ...
        <div className="mt-1 text-sm space-y-1">
          ...
          <div className="mt-1">
              ...
              {(typeof currentAreaMap === 'string' && currentAreaMap.trim() !== "") ? (
                  <div className="area-map-container custom-scrollbar">
                      <pre className="area-map-text">{currentAreaMap}</pre>
                  </div>
              ) : (
                  <p className="text-sm text-slate-500 italic mt-1">当前区域地图未探索或不可用</p>
              )}
          </div>
          <button
