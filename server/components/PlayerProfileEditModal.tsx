...
const PlayerProfileEditModal: React.FC<PlayerProfileEditModalProps> = ({ isOpen, onClose, currentProfile, onSave }) => {
  ...
  const [editableAge, setEditableAge] = useState<string>(currentProfile.age?.toString() || '');
  const [editableDescription, setEditableDescription] = useState(currentProfile.description || '');
  ...
  return (
    <div className="modal-overlay-base" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="playerProfileEditModalTitle">
      <div className="modal-content-base max-w-lg" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header-base">
          <h2 id="playerProfileEditModalTitle" className="text-xl font-bold text-yellow-300">编辑角色信息</h2>
          <button onClick={onClose} className="modal-close-button-base" aria-label="关闭编辑角色信息">
            &times;
          </button>
        </div>
        <form onSubmit={handleSave} className="modal-body-base custom-scrollbar space-y-4">
          <div>
            <label htmlFor="editPlayerName" className="label-text">姓名:</label>
            <input
              type="text"
              id="editPlayerName"
              value={editableName}
          ...
          <div>
            <label htmlFor="editPlayerGender" className="label-text">性别:</label>
            <select
              id="editPlayerGender"
          ...
          <div>
            <label htmlFor="editPlayerAge" className="label-text">年龄:</label>
            <input
              type="number"
              id="editPlayerAge"
              value={editableAge}
          ...
          <div>
            <label htmlFor="editPlayerDescription" className="label-text">人物说明 (性格/背景):</label>
            <textarea
          ...
          <div className="pt-4 border-t border-purple-600/30 flex justify-end gap-3">
            <button type="button" onClick={onClose} className="choice-button secondary">
              取消
            </button>
