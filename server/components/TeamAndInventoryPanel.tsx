...
const SectionTitle: React.FC<{children: React.ReactNode, className?: string}> = ({children, className}) => (
  <h3 className={`text-lg font-semibold mb-2 text-purple-600 ${className || ''}`}>{children}</h3>
);
  ...
  return (
    <>
      <div className="sidebar-panel h-full overflow-y-auto custom-scrollbar flex flex-col">
        <SectionTitle className="border-b border-purple-300/70 pb-2">队伍与背包</SectionTitle>
        <button
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 inline-block" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M10 2a1 1 0 00-1 1v1H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V6a2 2 0 00-2-2h-3V3a1 1 0 00-1-1zm3 8a1 1 0 100-2 1 1 0 000 2zM7 10a1 1 0 100-2 1 1 0 000 2zm3 5a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
          </svg>
          背包
        </button>
        
        <SectionTitle className="border-t border-purple-300/70 pt-3">同行宝可梦</SectionTitle>
        {playerTeam.length > 0 ? (
            ...
            {playerTeam.map(pokemon => (
              pokemon && pokemon.instanceId ?
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onOpenPokemonDetailModal(pokemon);}}
