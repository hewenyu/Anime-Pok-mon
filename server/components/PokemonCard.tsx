...
const ResourceBar: React.FC<{
  ...
  return (
    <div
      className="w-full bg-gray-300/70 rounded-full border border-gray-400/30 shadow-inner overflow-hidden my-0.5"
...
const PokemonCard: React.FC<PokemonCardProps> = ({
  ...
  const renderStatusConditions = (conditions: ActiveStatusCondition[]) => {
    ...
    return (
      <div
        className={`flex gap-1 mt-1 ${isPlayerCard && isBattleCard ? 'justify-end' : 'justify-start'}`}
      >
        {conditions.map(activeCond => {
          ...
          const statusSpecificColor =
            condInfo.colorClass || 'bg-gray-500 text-white';
          return (
            <span
  ...
  const StatsBlock: React.FC = () => (
    <div
      className={`flex-grow ${isPlayerCard ? 'text-right pr-1' : 'text-left pl-1'}`}
    >
      <div
        className={`flex ${isPlayerCard ? 'justify-end' : 'justify-start'} items-baseline mb-1`}
      >
        <h3
          className={`text-lg md:text-xl font-bold ${nameColor} mr-2 drop-shadow-sm`}
        >
          {name}
        </h3>
        <span className={`text-sm ${levelColor} font-semibold`}>
          Lv.{level}
        </span>
      </div>
      ...
      <ResourceBar
        current={currentHp}
        max={maxHp}
        colorClass={getHpColorClass(currentHp, maxHp)}
        label="HP"
        textClass={statsTextColor}
      />
      <p className={`text-xs font-medium ${statsTextColor} mt-0.5 mb-1`}>
        {currentHp} / {maxHp} HP
      </p>
      ...
      {!isFainted && !isBattleCard && (
        <>
          <ResourceBar
            ...
            textClass={statsTextColor}
          />
          <p className={`text-xs font-medium ${statsTextColor} mt-0.5`}>
      ...
      <div
        className={`flex gap-1.5 mt-2 ${isPlayerCard ? 'justify-end' : 'justify-start'}`}
      >
  ...
  const ImageBlock: React.FC = () => (
    <div
      className={`relative ${isBattleCard ? (isPlayerCard ? 'w-24 h-24 md:w-28 md:h-28 lg:w-32 lg:h-32' : 'w-20 h-20 md:w-24 md:h-24 lg:w-28 lg:h-28') : 'w-16 h-16 md:w-20 md:h-20'} flex-shrink-0 bg-gray-100/50 rounded-md border border-gray-300/50 flex items-center justify-center overflow-hidden`}
    >
  ...
  if (!isBattleCard) {
    ...
    return (
      <div
        className={`p-2.5 rounded-lg shadow-md bg-white/70 border border-purple-200/60 flex flex-col gap-2 ${isFainted ? 'opacity-70' : ''}`}
      >
        <div className="flex items-center gap-3">
          <ImageBlock />
          <div className="flex-grow">
            <h4 className={`text-md font-semibold ${nameColor}`}>
              {name} <span className={`text-xs ${levelColor}`}>Lv.{level}</span>
            </h4>
            {renderStatusConditions(statusConditions)}
            <ResourceBar
              current={currentHp}
              ...
              colorClass={getHpColorClass(currentHp, maxHp)}
              label="HP"
              barHeight="h-1.5 md:h-2"
              textClass={nonBattleStatsTextColor}
            />
            <p className={`text-xs ${nonBattleStatsTextColor} mt-0.5 mb-1`}>
              {currentHp} / {maxHp} HP
            </p>
            ...
            <ResourceBar
              ...
              textClass={nonBattleStatsTextColor}
            />
            <p className={`text-xs ${nonBattleStatsTextColor} mt-0.5`}>
              {currentXp} / {xpToNextLevel} XP
            </p>
