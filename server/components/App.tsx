...
const App: React.FC = () => {
  ...
  const memoizedHandleSendPlayerMessageToNPC = (
    ...
    if (selectedNPCForChat && selectedNPCForChat.id === npcId) {
      ...
      // NPC context mismatch or no NPC selected - this is a developer error
      // but should not crash the app in production
      return;
    }
  };
  ...
  const renderContent = () => {
    ...
    switch (gameState.gameMode) {
      ...
      case GameMode.CUSTOMIZE_RANDOM_START:
        return (
          <CustomizeRandomStartScreen
            ...
            aiNarrativeFromApp={
              gameState.aiLoadingStatus.status === 'loading'
                ? gameState.aiLoadingStatus.message || '处理中...'
                : gameState.aiLoadingStatus.status === 'retrying_format_error'
                  ? gameState.aiLoadingStatus.message || '重试中...'
                  : gameState.currentAIScene?.narrative ||
                    (isLoadingAI && !gameState.customizationAssistantResponse
                      ? '处理中...'
                      : '自定义你的冒险开始！')
            }
      ...
      case GameMode.ADVENTURE: {
        ...
        if (
          isLoadingAI &&
          (currentStaticSeg?.isAIHandoff || gameState.currentAIScene === null)
        ) {
        ...
        if (
          effectiveStaticContent?.triggerBattle &&
          (!effectiveStaticContent.choices ||
            effectiveStaticContent.choices.length === 0)
        ) {
          effectiveStaticContent.choices = [
            {
              text: `与 ${effectiveStaticContent.triggerBattle.enemyPokemon.name} 开始战斗! (触发战斗)`,
              isBattleTrigger: true,
              actionTag: `STATIC_BATTLE_TRIGGER_${effectiveStaticContent.id}`,
            },
          ];
        }
        ...
        return (
          <AdventureView
            ...
            currentAIContent={
              displayAIContent ? gameState.currentAIScene : null
            }
      ...
      case GameMode.BATTLE: {
        ...
        const anyConsciousPokemon = gameState.playerTeam.find(
          p => !p.isFainted && p.isPlayerOwned
        );

        let initialPlayerPokemonIdForBattle =
          gameState.currentBattlePlayerPokemonId;
        // If the designated Pokémon is fainted or not found, pick the first available one.
        if (
          !firstPlayerPokemonForBattle &&
          anyConsciousPokemon &&
          anyConsciousPokemon.instanceId
        ) {
          initialPlayerPokemonIdForBattle = anyConsciousPokemon.instanceId;
        }

        if (
          !initialPlayerPokemonIdForBattle ||
          !anyConsciousPokemon ||
    ...
    <>
      ...
      {showGlobalMapModal && (
        <GlobalMapModal
          ...
          onClose={toggleGlobalMapModal}
          globalMapData={gameState.globalAreaMap}
        />
      )}
      {showNPCListModal && (
        <NPCListModal
          isOpen={showNPCListModal}
          onClose={toggleNPCListModal}
          npcs={gameState.knownNPCs}
          onSelectNPC={handleSelectNPCToChat}
        />
      )}
      {selectedNPCForChat && (
        <NPCChatModal
          ...
          isLoadingAI={
            npcInteractionLoading ||
            (gameState.aiLoadingStatus.status === 'loading' &&
              gameState.aiLoadingStatus.message === '获取NPC对话...')
          }
