...
import { StorySegment, PlayerProfile, AIStoryResponse, GameState, Pokemon, StoryChoice, AIStoryChoice, LoadingStatus } from '../types';
...
const AdventurePanel: React.FC<AdventurePanelProps> = ({
  ...
  const [persistentImageUrl, setPersistentImageUrl] = useState<string | undefined>(undefined);
  ...
  if (currentAIContent) {
    ...
    displaySpeaker = typeof seg.speaker === 'function' ? seg.speaker(playerProfile) : seg.speaker;
    displayText = typeof seg.text === 'function' ? seg.text(playerProfile, playerTeam) : seg.text;
  } else {
    if (isLoading) {
        const lastAiNarrativeEntry = [...gameState.chatHistory].reverse().find(entry => entry.type === 'ai' || entry.type === 'static');
  ...
  if (!isLoading) {
    ...
    } else if (currentStaticContent?.choices && currentStaticContent.choices.length > 0) {
        displayChoicesElements = (
          <div className="space-y-3">
            {currentStaticContent.choices.map((choice, index) => {
              const isDisabled = choice.disabled ? choice.disabled(gameState) : false;
              let buttonText = choice.text;
              if (choice.isBattleTrigger || choice.actionTag?.startsWith('STATIC_BATTLE_TRIGGER_')) {
                buttonText += " (触发战斗)";
              }
              return (
                <button
  ...
  const lastPlayerInputAction = (gameState.chatHistory.length > 0 && 
    gameState.chatHistory[gameState.chatHistory.length - 1].type === 'player_input' &&
    (Date.now() - gameState.chatHistory[gameState.chatHistory.length - 1].timestamp < 8000) 
  ) ? gameState.chatHistory[gameState.chatHistory.length - 1] : null;
  ...
  return (
    <div className="flex flex-col h-full adventure-text-box"> 
      <div className="flex-grow mb-4 overflow-y-auto p-1 custom-scrollbar"> 
        ...
        {displaySpeaker && <p className="font-bold text-xl mb-2 text-yellow-300 drop-shadow-sm">{displaySpeaker}:</p>}
        <p className="text-lg whitespace-pre-wrap leading-relaxed clear-both text-indigo-50">{displayText}</p>
        
        {lastPlayerInputAction && (isLoading || (currentAIContent && Date.now() - lastPlayerInputAction.timestamp < 3000)) && (
            <p className="text-lg whitespace-pre-wrap leading-relaxed italic text-cyan-300 mt-3 animate-message-appear clear-both">
              <span className="font-bold not-italic text-yellow-300">{lastPlayerInputAction.speaker}:</span> &gt; {lastPlayerInputAction.narrative}
            </p>
        )}
      ...
      {!isLoading && displayChoicesElements && <div className="mb-4">{displayChoicesElements}</div>}

      { (currentAIContent || (currentStaticContent && currentStaticContent.isAIHandoff && !isLoading) || (!currentStaticContent && !currentAIContent && !isLoading)) && (
        <form onSubmit={handleCustomInputSubmit} className="mt-auto pt-4 border-t border-purple-600/50">
