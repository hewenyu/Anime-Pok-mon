...
interface CustomizeRandomStartScreenProps {
  ...
  onSendAssistantMessage: (
    messageText: string,
    assistantChatHistory: ChatHistoryEntry[],
    actionTag?: string
  ) => void;
  requestDynamicTimeSuggestion: (
    profileData: ProfileDataForTimeSuggestion
  ) => void; // New prop
...
const parseTimestampToUserDateTimeInput = (
  timestamp: number
): UserDateTimeInput => {
...
const CustomizeRandomStartScreen: React.FC<CustomizeRandomStartScreenProps> = ({
  ...
  aiNarrativeFromApp,
  onReRollFullProfile,
  onRequestAddRandomStarterViaMainButton,
  onRequestNewItemViaMainButton,
  ...
  const [editableAge, setEditableAge] = useState<number | undefined>(
    currentProfile.age
  );
  const [editableDescription, setEditableDescription] = useState<string>(
    currentProfile.description || ''
  );
  const [editableStamina, setEditableStamina] = useState<number | undefined>(
    currentProfile.stamina
  );
  const [editableMaxStamina, setEditableMaxStamina] = useState<
    number | undefined
  >(currentProfile.maxStamina);
  const [editableEnergy, setEditableEnergy] = useState<number | undefined>(
    currentProfile.energy
  );
  const [editableMaxEnergy, setEditableMaxEnergy] = useState<
    number | undefined
  >(currentProfile.maxEnergy);
  const [editableHealthStatus, setEditableHealthStatus] = useState<string>(
  ...
  const [editableObjective, setEditableObjective] =
    useState<string>(currentObjective);
  ...
  const [localEditableItems, setLocalEditableItems] = useState<InventoryItem[]>(
    [...currentItems]
  );
  const [expandedInitialItemId, setExpandedInitialItemId] = useState<
    string | null
  >(null);
  ...
  const isLoadingMainAIForInitialScreen =
    aiLoadingStatus.status !== 'idle' &&
    aiLoadingStatus.message !== '助手思考中...' &&
    aiLoadingStatus.message !== '助手理解中...';
  const isAssistantLoading =
    aiLoadingStatus.status !== 'idle' &&
    !!aiLoadingStatus.message?.startsWith('助手');
  const isMainFormLoading =
    aiLoadingStatus.status !== 'idle' &&
    !aiLoadingStatus.message?.startsWith('助手');
  ...
  useEffect(() => {
    ...
    } else if (!aiSuggestedGameStartTime && !initialProfileGenerated) {
      // Only default to current time if no AI suggestion and profile not yet generated
      updateTimeFieldsFromTimestamp(new Date().getTime());
    }
  }, [initialProfileGenerated, aiSuggestedGameStartTime]);

  useEffect(() => {
    setEditableProfileName(currentProfile.name || '');
  }, [currentProfile.name]);
  useEffect(() => {
    setEditableGender(currentProfile.gender || '男');
  }, [currentProfile.gender]);
  useEffect(() => {
    setEditableAge(currentProfile.age);
  }, [currentProfile.age]);
  useEffect(() => {
    setEditableDescription(currentProfile.description || '');
  }, [currentProfile.description]);
  useEffect(() => {
    setEditableStamina(currentProfile.stamina);
  }, [currentProfile.stamina]);
  useEffect(() => {
    setEditableMaxStamina(currentProfile.maxStamina);
  }, [currentProfile.maxStamina]);
  useEffect(() => {
    setEditableEnergy(currentProfile.energy);
  }, [currentProfile.energy]);
  useEffect(() => {
    setEditableMaxEnergy(currentProfile.maxEnergy);
  }, [currentProfile.maxEnergy]);
  useEffect(() => {
    setEditableHealthStatus(currentProfile.healthStatus || '');
  }, [currentProfile.healthStatus]);

  useEffect(() => {
    setLocalEditableItems([...currentItems]);
  }, [currentItems]);
  useEffect(() => {
    setEditableMoney(currentMoney);
  }, [currentMoney]);
  useEffect(() => {
    setEditableLocation(currentLocation);
  }, [currentLocation]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (assistantAIResponse) {
      const mainAiNarrative =
        assistantAIResponse.narrative || '这是为你定制的档案建议：';
      let newEntry: ChatHistoryEntry;

      if (
        assistantAIResponse.choices &&
        assistantAIResponse.choices.some(c =>
          c.actionTag?.startsWith('ASSIST_PROFILE_CUSTOMIZATION_SET_GAME_TIME:')
        )
      ) {
        ...
        assistantAIResponse.suggestedFullProfileData &&
        assistantAIResponse.events
      ) {
        if (
          assistantAIResponse.suggestedFullProfileData.suggestedGameStartTime
        ) {
          updateTimeFieldsFromTimestamp(
            assistantAIResponse.suggestedFullProfileData.suggestedGameStartTime
          );
        }
        newEntry = {
        ...
        if (
          assistantAIResponse.events &&
          assistantAIResponse.events.length > 0
        ) {
          const firstEvent = assistantAIResponse.events[0];
          if (
            firstEvent.type === 'PRESENT_SUGGESTED_POKEMON_DETAILS' &&
            firstEvent.pokemonDetails
          ) {
            const suggestedPokemon = sanitizePokemonFn(
              firstEvent.pokemonDetails
            );
            ...
            newEntry.confirmationChoices = assistantAIResponse.choices?.filter(
              c => c.actionTag !== 'ACCEPT_ASSISTANT_FULL_PROFILE_SUGGESTION'
            ) || [
              {
                text: '确认添加这只宝可梦',
                actionTag: `ASSIST_PROFILE_CUSTOMIZATION_CONFIRM_ADD_POKEMON`,
              },
              {
                text: '换一只其他的',
                actionTag: `ASSIST_PROFILE_CUSTOMIZATION_REQUEST_NEW_POKEMON`,
              },
              {
                text: '不用了，谢谢',
                actionTag: `ASSIST_PROFILE_CUSTOMIZATION_CANCEL_SUGGESTION`,
              },
            ];
          } else if (
            firstEvent.type === 'PRESENT_SUGGESTED_ITEM_DETAILS' &&
            firstEvent.itemDetails
          ) {
            const suggestedItem = sanitizeItemFn(firstEvent.itemDetails);
            newEntry.narrative = mainAiNarrative || `我为你找到了这个道具：`;
            newEntry.type = 'assistant_suggestion';
            newEntry.suggestedItemForConfirmation = {
              ...suggestedItem,
              quantity: firstEvent.quantity || suggestedItem.quantity || 1,
            };
            newEntry.confirmationChoices = assistantAIResponse.choices?.filter(
              c => c.actionTag !== 'ACCEPT_ASSISTANT_FULL_PROFILE_SUGGESTION'
            ) || [
              {
                text: '确认添加这些道具',
                actionTag: `ASSIST_PROFILE_CUSTOMIZATION_CONFIRM_ADD_ITEM`,
              },
              {
                text: '换一些其他的',
            ...
            firstEvent.type === 'PRESENT_SUGGESTED_LOCATION_DETAILS' &&
            firstEvent.suggestedLocationDetails
          ) {
            newEntry.narrative =
              mainAiNarrative ||
              `建议将初始位置设为：${firstEvent.suggestedLocationDetails.newLocation}`;
            newEntry.type = 'assistant_suggestion';
            newEntry.suggestedLocationDetails =
              firstEvent.suggestedLocationDetails;
            newEntry.confirmationChoices = assistantAIResponse.choices?.filter(
              c => c.actionTag !== 'ACCEPT_ASSISTANT_FULL_PROFILE_SUGGESTION'
            ) || [
              {
                text: `确认设为 ${firstEvent.suggestedLocationDetails.newLocation}`,
                actionTag: `ASSIST_PROFILE_CUSTOMIZATION_SET_LOCATION:${firstEvent.suggestedLocationDetails.newLocation}`,
              },
              {
                text: '换一个地方',
                actionTag: `ASSIST_PROFILE_CUSTOMIZATION_REQUEST_NEW_LOCATION`,
              },
              {
                text: '不用了，谢谢',
                actionTag: `ASSIST_PROFILE_CUSTOMIZATION_CANCEL_SUGGESTION`,
  ...
  useEffect(() => {
    if (assistantChatBodyRef.current) {
      assistantChatBodyRef.current.scrollTop =
        assistantChatBodyRef.current.scrollHeight;
    }
  }, [assistantChatHistory]);

  const handleNumericProfileChange = (
    setter: React.Dispatch<React.SetStateAction<number | undefined>>,
    value: string,
    fieldName: keyof PlayerProfile
  ) => {
  ...
  const handleStringProfileChange = (
  ...
  const handleItemQuantityChangeInForm = (
    itemId: string,
    quantityStr: string
  ) => {
    const quantity = parseInt(quantityStr, 10);
    const validQuantity = isNaN(quantity) || quantity < 0 ? 0 : quantity;
    setLocalEditableItems(prevItems =>
      prevItems.map(item =>
        item.id === itemId ? { ...item, quantity: validQuantity } : item
      )
    );
    onUpdateCustomizationDirectly('itemQtyUpdate', {
      itemId,
  ...
  const handleSubmitForm = () => {
    ...
    const finalProfile: PlayerProfile = {
      ...
      stamina: editableStamina ?? 100,
      maxStamina: editableMaxStamina ?? 100,
      energy: editableEnergy ?? 100,
      maxEnergy: editableMaxEnergy ?? 100,
  ...
  const handleSendAssistantChatMessage = (
    text?: string,
    actionTag?: string
  ) => {
    ...
    if (!isAssistantLoading) {
      const playerEntry: ChatHistoryEntry = {
        id: `assist-player-${Date.now()}`,
        timestamp: Date.now(),
        speaker: editableProfileName || '你',
        narrative: messageToSend,
        type: 'player_input',
      };
  ...
  const handleAssistantSuggestionConfirmation = (
    ...
    suggestionMessage: ChatHistoryEntry & {
    ...
    if (choice.actionTag === 'ACCEPT_ASSISTANT_FULL_PROFILE_SUGGESTION') {
      currentTeam.forEach(p => {
        if (p.instanceId)
          onUpdateCustomizationDirectly('teamMemberRemove', p.instanceId);
      });
      ...
      const profileUpdateMessages: string[] = [];
      if (suggestionMessage.suggestedFullProfileData) {
        const data = suggestionMessage.suggestedFullProfileData;
        if (data.name !== undefined) {
          setEditableProfileName(data.name);
          onUpdateCustomizationDirectly('profileFieldUpdate', {
            field: 'name',
            value: data.name,
          });
          profileUpdateMessages.push(`姓名: ${data.name}`);
        }
        if (data.gender !== undefined) {
          ...
          profileUpdateMessages.push(`性别: ${data.gender}`);
        }
        if (data.age !== undefined) {
          ...
          profileUpdateMessages.push(`年龄: ${data.age}`);
        }
        if (data.description !== undefined) {
          ...
          profileUpdateMessages.push(`说明已更新`);
        }
        if (data.stamina !== undefined) {
          setEditableStamina(data.stamina);
          onUpdateCustomizationDirectly('profileFieldUpdate', {
            field: 'stamina',
            value: data.stamina,
          });
        ...
        if (data.maxStamina !== undefined) {
          setEditableMaxStamina(data.maxStamina);
          onUpdateCustomizationDirectly('profileFieldUpdate', {
            field: 'maxStamina',
        ...
        if (data.maxEnergy !== undefined) {
          setEditableMaxEnergy(data.maxEnergy);
          onUpdateCustomizationDirectly('profileFieldUpdate', {
            field: 'maxEnergy',
        ...
        if (data.healthStatus !== undefined) {
          setEditableHealthStatus(data.healthStatus);
          onUpdateCustomizationDirectly('profileFieldUpdate', {
            field: 'healthStatus',
        ...
        if (data.suggestedGameStartTime) {
          ...
          onUpdateCustomizationDirectly(
            'gameTimeUpdate',
            parseTimestampToUserDateTimeInput(data.suggestedGameStartTime)
          );
      ...
      if (suggestionMessage.events) {
        suggestionMessage.events.forEach(event => {
          if (
            ...
            const itemWithQty = {
              ...item,
              quantity: event.quantity || item.quantity || 1,
            };
            onUpdateCustomizationDirectly('itemAdd', itemWithQty);
            profileUpdateMessages.push(
              `道具: ${item.name} x${itemWithQty.quantity}`
            );
          }
        });
      }
      ...
      choice.actionTag === 'ASSIST_PROFILE_CUSTOMIZATION_CONFIRM_ADD_POKEMON' &&
      suggestionMessage.suggestedPokemonForConfirmation
    ) {
      onUpdateCustomizationDirectly(
        'teamMemberAdd',
        suggestionMessage.suggestedPokemonForConfirmation
      ...
      choice.actionTag === 'ASSIST_PROFILE_CUSTOMIZATION_CONFIRM_ADD_ITEM' &&
      suggestionMessage.suggestedItemForConfirmation
    ) {
      onUpdateCustomizationDirectly(
        'itemAdd',
        suggestionMessage.suggestedItemForConfirmation
      ...
      choice.actionTag?.startsWith(
        'ASSIST_PROFILE_CUSTOMIZATION_SET_PROFILE_FIELD:'
      )
    ) {
          ...
          handleNumericProfileChange(
            fieldName === 'age'
              ...
              : fieldName === 'stamina'
                ? setEditableStamina
                : fieldName === 'maxStamina'
                  ? setEditableMaxStamina
                  : fieldName === 'energy'
                    ? setEditableEnergy
                    : setEditableMaxEnergy,
            value,
            fieldName
          );
      ...
      choice.actionTag?.startsWith('ASSIST_PROFILE_CUSTOMIZATION_SET_MONEY:')
    ) {
      const amountStr = choice.actionTag.substring(
        'ASSIST_PROFILE_CUSTOMIZATION_SET_MONEY:'.length
      );
      ...
      choice.actionTag?.startsWith('ASSIST_PROFILE_CUSTOMIZATION_SET_LOCATION:')
    ) {
      const locationName = choice.actionTag.substring(
        'ASSIST_PROFILE_CUSTOMIZATION_SET_LOCATION:'.length
      );
      ...
      choice.actionTag?.startsWith(
        'ASSIST_PROFILE_CUSTOMIZATION_SET_OBJECTIVE:'
      )
    ) {
      const objectiveText = choice.actionTag.substring(
        'ASSIST_PROFILE_CUSTOMIZATION_SET_OBJECTIVE:'.length
      );
      ...
      choice.actionTag?.startsWith(
        'ASSIST_PROFILE_CUSTOMIZATION_SET_GAME_TIME:'
      )
    ) {
      ...
      if (parts.length === 6) {
        ...
        if (![year, monthJs, day, hour, minute].some(isNaN)) {
          const userTimeInput: UserDateTimeInput = {
            year,
            month: monthJs + 1,
            day,
            hour,
    ...
    if (systemMessageText) {
      const confirmMsg: ChatHistoryEntry = {
        id: `assist-sys-${Date.now()}`,
        timestamp: Date.now(),
        speaker: '系统',
        narrative: systemMessageText,
        type: 'system',
      };
  ...
  const renderFormLoadingOverlay = () => (
    <div className="absolute inset-0 bg-white/70 backdrop-blur-sm flex flex-col items-center justify-center z-50 rounded-lg">
      <p className="text-purple-600 text-lg italic animate-pulse">
        {aiLoadingStatus.message || '处理中...'}
      </p>
    </div>
  );
  ...
  if (
    !initialProfileGenerated &&
    isLoadingMainAIForInitialScreen &&
    !assistantAIResponse
  ) {
    return (
    ...
    <>
      <div className="min-h-screen flex flex-col items-center justify-start p-4 overflow-y-auto">
        <div className="adventure-text-box w-full max-w-6xl mt-2 mb-3 relative">
          <h1 className="text-2xl font-bold mb-1 text-pink-600 text-center">
            自定义你的冒险启程
          </h1>
        ...
        <div className="flex flex-col md:flex-row w-full max-w-6xl gap-3 flex-grow">
          <div className="sidebar-panel md:w-3/5 lg:w-2/3 p-3 md:p-4 custom-scrollbar overflow-y-auto relative">
            ...
            <h3 className="text-lg font-semibold text-sky-700 border-b border-purple-300/70 pb-1 mb-1.5">
              角色信息
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-2.5 mb-3">
              <div>
                <label htmlFor="randName" className="label-text text-sm">
                  姓名:
                </label>
                <input
                  type="text"
                  ...
                  className="input-field w-full mt-0.5 text-sm p-1.5"
                  disabled={isMainFormLoading}
                  maxLength={15}
                />
              </div>
              <div>
                <label htmlFor="randGender" className="label-text text-sm">
                  性别:
                </label>
                <select
                  id="randGender"
                  ...
                  onChange={e =>
                    handleGenderProfileChange(e.target.value as PlayerGender)
                  }
                  className="input-field w-full mt-0.5 text-sm p-1.5"
                  disabled={isMainFormLoading}
                >
              ...
              <div>
                <label htmlFor="randAge" className="label-text text-sm">
                  年龄:
                </label>
                <input
                  type="number"
            ...
            <div className="mb-2">
              <label htmlFor="randDescription" className="label-text text-sm">
                人物说明 (性格/背景):
              </label>
              <textarea
                id="randDescription"
                ...
                className="input-field w-full mt-0.5 text-sm p-1.5 h-20 custom-scrollbar"
                disabled={isMainFormLoading}
                maxLength={150}
                placeholder="例如：一个充满好奇心，梦想成为宝可梦大师的少年。"
              ></textarea>
              <button
                onClick={onRequestGeneratePlayerDescription}
                className="choice-button secondary text-xs w-full py-1 mt-1"
                disabled={isMainFormLoading}
              >
                AI 生成说明
              </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-2.5 mb-2">
              <div>
                <label htmlFor="randStamina" className="label-text text-sm">
                  体力:
                </label>
                <input
                  type="number"
                  id="randStamina"
                  value={editableStamina ?? ''}
                  onChange={e =>
                    handleNumericProfileChange(
                      setEditableStamina,
                      e.target.value,
              ...
              <div>
                <label htmlFor="randMaxStamina" className="label-text text-sm">
                  最大体力:
                </label>
                <input
                  type="number"
                  id="randMaxStamina"
                  value={editableMaxStamina ?? ''}
                  onChange={e =>
                    handleNumericProfileChange(
                      setEditableMaxStamina,
                      e.target.value,
              ...
              <div>
                <label htmlFor="randEnergy" className="label-text text-sm">
                  精力:
                </label>
                <input
                  type="number"
                  id="randEnergy"
                  value={editableEnergy ?? ''}
                  onChange={e =>
                    handleNumericProfileChange(
                      setEditableEnergy,
                      e.target.value,
              ...
              <div>
                <label htmlFor="randMaxEnergy" className="label-text text-sm">
                  最大精力:
                </label>
                <input
                  type="number"
                  id="randMaxEnergy"
                  value={editableMaxEnergy ?? ''}
                  onChange={e =>
                    handleNumericProfileChange(
                      setEditableMaxEnergy,
                      e.target.value,
            ...
            <div className="mb-2">
              <label htmlFor="randHealth" className="label-text text-sm">
                健康状况:
              </label>
              <input
                type="text"
                id="randHealth"
                value={editableHealthStatus}
                onChange={e =>
                  handleStringProfileChange(
                    setEditableHealthStatus,
                    e.target.value,
            ...
            <h3 className="text-lg font-semibold text-sky-700 border-b border-purple-300/70 pb-1 mb-1.5 mt-2">
              冒险设定
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-x-4 gap-y-2.5">
              <div className="col-span-full md:col-span-1">
                <label htmlFor="randLocation" className="label-text text-sm">
                  初始位置:
                </label>
                <input
                  type="text"
              ...
              <div className="col-span-full md:col-span-1">
                <label htmlFor="randMoney" className="label-text text-sm">
                  初始金钱:
                </label>
                <input
                  type="number"
                  ...
                  value={editableMoney}
                  onChange={e =>
                    setEditableMoney(parseInt(e.target.value, 10) || 0)
                  }
                  className="input-field w-full mt-0.5 text-sm p-1.5"
                  disabled={isMainFormLoading}
                />
              ...
              <div className="col-span-full">
                <label htmlFor="randObjective" className="label-text text-sm">
                  初始目标:
                </label>
                <input
                  type="text"
            ...
            <div className="mt-2 pt-2 border-t border-purple-300/60">
              <label className="label-text text-sm mb-1 block">
                游戏开始时间 (年请填公元，负数表示公元前):
              </label>
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-x-2 gap-y-1.5 items-end">
                <input
                  type="text"
                  placeholder="年 (如 2024, -100)"
                  value={editableYear}
                  onChange={e => setEditableYear(e.target.value)}
                  className="input-field text-xs p-1 col-span-2 sm:col-span-2"
                ...
                <input
                  ...
                  placeholder="月 (1-12)"
                  value={editableMonth}
                  onChange={e => setEditableMonth(parseInt(e.target.value))}
                  min="1"
                  max="12"
                ...
                <input
                  ...
                  placeholder="日 (1-31)"
                  value={editableDay}
                  onChange={e => setEditableDay(parseInt(e.target.value))}
                  min="1"
                  max="31"
                  className="input-field text-xs p-1"
                  disabled={isMainFormLoading || isAssistantLoading}
                ...
                <input
                  ...
                  placeholder="时 (0-23)"
                  value={editableHour}
                  onChange={e => setEditableHour(parseInt(e.target.value))}
                  min="0"
                  max="23"
                  className="input-field text-xs p-1"
                  disabled={isMainFormLoading || isAssistantLoading}
                ...
                <input
                  ...
                  onChange={e => setEditableMinute(parseInt(e.target.value))}
                  min="0"
                  max="59"
                  className="input-field text-xs p-1"
                  disabled={isMainFormLoading || isAssistantLoading}
            ...
            <div className="mt-3">
              ...
              {currentTeam.length > 0 ? (
                <div className="space-y-2">
                  {currentTeam.map(pokemon => (
                    <div
                      ...
                      className="p-1.5 bg-purple-50/50 rounded"
                    >
                      <div
                        onClick={() => setPokemonToViewInModal(pokemon)}
                        className="pokemon-sidebar-card-clickable cursor-pointer"
                      >
                        <PokemonCard
                          pokemon={pokemon}
                          isBattleCard={false}
                      ...
                      <button
                        onClick={() =>
                          handleRemoveFromTeamViaForm(pokemon.instanceId)
                        }
                        className="choice-button text-xs w-full mt-1 py-1"
                        disabled={isMainFormLoading}
                      >
                        移除这只
                      </button>
                ...
                !isMainFormLoading && (
                  <p className="text-gray-500 my-1.5 text-sm">
                    沒有初始宝可梦。
                  </p>
                )
              )}
              <button
                onClick={onRequestAddRandomStarterViaMainButton}
                className="choice-button secondary text-xs sm:text-sm w-full py-1.5 mt-1"
                disabled={isMainFormLoading}
              >
                随机添加宝可梦
              </button>
            ...
            <div className="mt-3">
              ...
              {localEditableItems.length > 0 ? (
                <ul className="space-y-2 text-sm">
                  {localEditableItems.map(item => (
                    <li
                      ...
                      onClick={() =>
                        item.description && toggleInitialItemExpansion(item.id)
                      }
                      onKeyDown={e => {
                        if (
                          (e.key === 'Enter' || e.key === ' ') &&
                          ...
                          toggleInitialItemExpansion(item.id);
                      }}
                      tabIndex={item.description ? 0 : -1}
                      role={item.description ? 'button' : undefined}
                      aria-expanded={
                        item.description
                          ? expandedInitialItemId === item.id
                          : undefined
                      }
                      aria-controls={
                        item.description
                          ? `initial-item-desc-${item.id}`
                          : undefined
                      }
                    >
                        ...
                        <div className="flex flex-col items-end gap-1 ml-2 flex-shrink-0">
                          <div className="flex items-center gap-1">
                            ...
                            <input
                              ...
                              value={item.quantity}
                              onChange={e =>
                                handleItemQuantityChangeInForm(
                                  item.id,
                                  e.target.value
                                )
                              }
                              className="input-field w-14 text-xs py-0.5 px-1"
                              disabled={isMainFormLoading}
                            />
                          </div>
                          <button
                      ...
                      {expandedInitialItemId === item.id &&
                        item.description && (
                          <div
                            id={`initial-item-desc-${item.id}`}
                            className="mt-2 pt-2 border-t border-purple-300/50 ml-0 md:ml-12"
                          >
                ...
                !isMainFormLoading && (
                  <p className="text-gray-500 my-1.5 text-sm">沒有初始道具。</p>
                )
              )}
              <button
                onClick={onRequestNewItemViaMainButton}
                className="choice-button secondary text-xs sm:text-sm w-full py-1.5 mt-2"
                disabled={isMainFormLoading}
              >
                随机添加道具
              </button>
          ...
          <div className="sidebar-panel md:w-2/5 lg:w-1/3 p-2 md:p-3 flex flex-col h-full">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-md font-semibold text-cyan-700">
                AI 定制助手
              </h3>
              <button
                onClick={handleRefreshAssistantChat}
                className="choice-button secondary text-xs px-2 py-1"
            ...
            >
              {assistantChatHistory.map(entry => (
                <div
                  key={entry.id}
                  className={`p-1.5 rounded-md ${entry.type === 'player_input' ? 'bg-sky-200/80 self-end ml-4' : 'bg-indigo-200/70 self-start mr-4'}`}
                >
                  <span className="font-bold block text-xs text-gray-600">
                    {entry.speaker}:
                  </span>
                  <p className="whitespace-pre-wrap text-gray-800 text-xs leading-snug">
                    {entry.narrative}
                  </p>

                  {entry.type === 'assistant_suggestion' &&
                    entry.suggestedFullProfileData &&
                    entry.events && (
                      <div className="mt-1 pt-1 border-t border-indigo-300/50">
                        <strong className="text-xs text-indigo-700 block mb-0.5">
                          建议的完整档案详情:
                        </strong>
                        <ul className="list-disc list-inside pl-1 space-y-0.5 text-xs text-gray-700 mb-1.5">
                          {entry.suggestedFullProfileData.name && (
                            <li>姓名: {entry.suggestedFullProfileData.name}</li>
                          )}
                          {entry.suggestedFullProfileData.gender && (
                            <li>
                              性别: {entry.suggestedFullProfileData.gender}
                            </li>
                          )}
                          {entry.suggestedFullProfileData.age !== undefined && (
                            <li>年龄: {entry.suggestedFullProfileData.age}</li>
                          )}
                          {entry.suggestedFullProfileData.description && (
                            <li>
                              说明: {entry.suggestedFullProfileData.description}
                            </li>
                          )}
                          {entry.suggestedFullProfileData.objective && (
                            <li>
                              目标: {entry.suggestedFullProfileData.objective}
                            </li>
                          )}
                          {entry.suggestedFullProfileData.location && (
                            <li>
                              地点: {entry.suggestedFullProfileData.location}
                            </li>
                          )}
                          {entry.suggestedFullProfileData.money !==
                            undefined && (
                            <li>
                              金钱: {entry.suggestedFullProfileData.money}
                            </li>
                          )}
                          {entry.suggestedFullProfileData
                        ...
                        {entry.events
                          ...
                          .map((pokemonEvent, idx) => {
                            const suggestedPokemon = sanitizePokemonFn(
                              pokemonEvent.pokemonDetails!
                            );
                            return (
                              <div
                                key={`fullprof-poke-${idx}`}
                                className="my-1 p-1.5 bg-white/50 rounded cursor-pointer hover:bg-white/80 transition-colors"
                                onClick={() =>
                                  setPokemonToViewInModal(suggestedPokemon)
                                }
                                role="button"
                                tabIndex={0}
                                onKeyDown={e => {
                        ...
                        {entry.events
                          .filter(
                            e =>
                              e.type === 'PRESENT_SUGGESTED_ITEM_DETAILS' &&
                              e.itemDetails
                          )
                          .map((itemEvent, idx) => {
                            const suggestedItem = sanitizeItemFn(
                              itemEvent.itemDetails!
                            );
                            const itemWithQty = {
                              ...suggestedItem,
                              quantity:
                            ...
                            return (
                              <div
                                key={`fullprof-item-${idx}`}
                                className="my-1 p-1.5 bg-white/50 rounded text-xs flex items-center gap-2"
                              >
                                {itemWithQty.imageUrl && (
                                  <img
                                    src={itemWithQty.imageUrl}
                                    alt={itemWithQty.name}
                                    className="w-8 h-8 object-contain rounded bg-gray-200/70 p-0.5 border border-gray-300"
                                    onError={e =>
                                      handleItemImageError(e, itemWithQty.name)
                                    }
                                  />
                                )}
                                <div className="flex-grow">
                                  ...
                                  x {itemWithQty.quantity}
                                  {itemWithQty.effectText && (
                                    <span className="block text-indigo-700 font-medium text-[0.7rem] leading-tight mt-0.5">
                                      {itemWithQty.effectText}
                                    </span>
                                  )}
                                  {itemWithQty.description &&
                                    itemWithQty.description !==
                                      itemWithQty.effectText && (
                                      <span className="block text-gray-600 italic text-[0.7rem] leading-tight mt-0.5">
                                        {itemWithQty.description}
                                      </span>
                                    )}
                                </div>
                  ...
                  {entry.type === 'assistant_suggestion' &&
                    !entry.suggestedFullProfileData &&
                    entry.suggestedLocationDetails && (
                      <div className="my-1 p-1.5 bg-white/50 rounded text-xs">
                        <p className="text-purple-700 font-semibold">
                          建议地点: {entry.suggestedLocationDetails.newLocation}
                        </p>
                        {entry.suggestedLocationDetails.mapData && (
                          <pre className="mt-1 text-xs font-mono bg-gray-200/50 p-1 rounded overflow-x-auto">
                            {entry.suggestedLocationDetails.mapData}
                          </pre>
                        )}
                      </div>
                  ...
                  {entry.type === 'assistant_suggestion' &&
                    ...
                    entry.suggestedPokemonForConfirmation && (
                      ...
                      >
                        ...
                        />
                        {entry.isClickableSuggestion && (
                          <p className="text-xs text-cyan-600 italic mt-0.5 text-center">
                            点击宝可梦卡片查看详情
                          </p>
                        )}
                      </div>
                  ...
                  {entry.type === 'assistant_suggestion' &&
                    !entry.suggestedFullProfileData &&
                    entry.suggestedItemForConfirmation && (
                      <div className="my-1 p-1.5 bg-white/50 rounded text-xs flex items-center gap-2">
                          ...
                          <img
                            ...
                            onError={e =>
                              handleItemImageError(
                                e,
                                entry.suggestedItemForConfirmation!.name
                              )
                            }
                          />
                        )}
                        <div className="flex-grow">
                          ...
                          x {entry.suggestedItemForConfirmation.quantity}
                          {entry.suggestedItemForConfirmation.effectText && (
                            <span className="block text-indigo-700 font-medium text-[0.7rem] leading-tight mt-0.5">
                              {entry.suggestedItemForConfirmation.effectText}
                            </span>
                          )}
                          {entry.suggestedItemForConfirmation.description &&
                            entry.suggestedItemForConfirmation.description !==
                              entry.suggestedItemForConfirmation.effectText && (
                              <span className="block text-gray-600 italic text-[0.7rem] leading-tight mt-0.5">
                                {entry.suggestedItemForConfirmation.description}
                  ...
                  {entry.confirmationChoices &&
                    entry.confirmationChoices.length > 0 && (
                      <div className="mt-1.5 space-y-1">
                        {entry.confirmationChoices.map(choice => (
                          <button
                            key={choice.actionTag}
                            onClick={() =>
                              handleAssistantSuggestionConfirmation(
                                choice,
                            ...
                            className="choice-button secondary w-full text-xs py-1 px-1.5"
                            disabled={isAssistantLoading}
                          >
                            {choice.text}
                          </button>
              ...
              {isAssistantLoading && (
                <p className="text-sky-600 italic text-xs animate-pulse">
                  {aiLoadingStatus.message || '助手处理中...'}
                </p>
              )}
            </div>
            <form
              onSubmit={e => {
                e.preventDefault();
                handleSendAssistantChatMessage();
              }}
            ...
            >
              <input
                ...
                placeholder="和AI聊聊你的想法..."
                className="input-field flex-grow text-sm p-1.5"
                disabled={isAssistantLoading}
              />
              <button
                type="submit"
                className="choice-button px-3 py-1.5 text-sm"
        ...
        <div className="w-full max-w-6xl mt-3 mb-2 flex flex-col sm:flex-row gap-3">
          <button
            onClick={onReRollFullProfile}
            className="choice-button secondary w-full py-2"
            disabled={isMainFormLoading}
          >
            {isMainFormLoading
              ? aiLoadingStatus.message || '处理中...'
          ...
          <button
            ...
            className="choice-button w-full py-2"
            disabled={
              isMainFormLoading ||
              !initialProfileGenerated ||
              isAssistantLoading
