...
import { Pokemon, PokemonType, IVs, PokemonMoveInstance, MoveEffect, NPC, InventoryItem, ItemEffect, ItemEffectType, StatusCondition } from '../types';
...
export const sanitizePokemonData = (aiPokemon: Partial<Pokemon>): Pokemon => {
  ...
  const xpToNextLevelDefault = Math.floor(Math.pow(level, 2.8)) + (level * 25) + 75;
  const xpToNextLevel = Math.max(1, aiPokemon.xpToNextLevel ?? xpToNextLevelDefault);
  ...
  const sanitizedTypes = (aiPokemon.types || [PokemonType.NORMAL])
    .map(t => typeof t === 'string' && isValidPokemonType(t) ? t : PokemonType.NORMAL)
  ...
  const sanitizedMoves = (aiPokemon.moves || []).map((move): PokemonMoveInstance => {
    ...
    const effects: MoveEffect[] = (move.effects || []).map(eff => ({
        ...
        chance: typeof eff.chance === 'number' ? Math.max(0, Math.min(1, eff.chance)) : undefined,
        statusCondition: eff.statusCondition,
        statChanges: eff.statChanges,
        healPercent: typeof eff.healPercent === 'number' ? eff.healPercent : undefined,
        damageHealRatio: typeof eff.damageHealRatio === 'number' ? eff.damageHealRatio : undefined,
        recoilPercent: typeof eff.recoilPercent === 'number' ? eff.recoilPercent : undefined,
        recoilFixedPercentMaxHp: typeof eff.recoilFixedPercentMaxHp === 'number' ? eff.recoilFixedPercentMaxHp : undefined,
        fixedDamage: typeof eff.fixedDamage === 'number' ? eff.fixedDamage : undefined,
        multiHitMin: typeof eff.multiHitMin === 'number' ? eff.multiHitMin : undefined,
        multiHitMax: typeof eff.multiHitMax === 'number' ? eff.multiHitMax : undefined,
        effectString: typeof eff.effectString === 'string' ? eff.effectString : undefined,
    }));

    return {
      name: move.name || "冲击",
      power: typeof move.power === 'number' ? move.power : 0,
      type: typeof move.type === 'string' && isValidPokemonType(move.type) ? move.type : PokemonType.NORMAL,
      category: category,
      basePP: basePP,
      currentPP: move.currentPP ?? basePP,
      description: typeof move.description === 'string' ? move.description : undefined,
      effects: effects.length > 0 ? effects : undefined,
      accuracy: typeof move.accuracy === 'number' ? Math.max(0, Math.min(100, move.accuracy)) : undefined,
  ...
  return {
    ...
    imageUrl: aiPokemon.imageUrl || "https://placehold.co/96x96/322D41/E0D8F0?text=?",
    ...
    instanceId: aiPokemon.instanceId || `${aiPokemon.id || 'pokemon'}-${Date.now()}-${Math.random().toString(36).substring(2,7)}`,
  };
};

export const sanitizeNPCData = (aiNPC: Partial<NPC>): NPC => {
  return {
    id: aiNPC.id || `npc_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
    name: aiNPC.name || "神秘人",
...
export const sanitizeItemData = (aiItem: Partial<InventoryItem>): InventoryItem => {
  const name = aiItem.name || "未知道具";
  let imageUrlToUse: string; 

  if (aiItem.imageUrl && (aiItem.imageUrl.startsWith('http://') || aiItem.imageUrl.startsWith('https://') || aiItem.imageUrl.startsWith('data:image'))) {
  ...
  if (name.includes("伤药") || name.toLowerCase().includes("potion")) {
    defaultEffect = { type: 'HEAL_HP', amount: name.includes("好伤药") || name.toLowerCase().includes("super") ? 50 : 20 };
  ...
  } else if (name.includes("解毒药") || name.toLowerCase().includes("antidote")) {
    defaultEffect = { type: 'CURE_STATUS', statusToCure: StatusCondition.POISONED };
    defaultCanUseInBattle = true;
    defaultTargetType = 'SELF_TEAM';
  }
  ...
  return {
    id: aiItem.id || `item-${name.replace(/\s+/g, '_').toLowerCase()}-${Date.now()}`,
    name: name,
