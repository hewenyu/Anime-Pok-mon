...
interface BattleActions {
  ...
  updateInventory: (
    updater: (inventory: InventoryItem[]) => InventoryItem[]
  ) => void;
  ...
  (set, _get) => ({
    ...
    initializeBattle: (
      ...
      const activePlayerPokemon =
        sanitizedPlayerTeam.find(
          p => p.instanceId === activePlayerPokemonId && !p.isFainted
        ) ||
        sanitizedPlayerTeam.find(p => !p.isFainted) ||
        null;
    ...
    updatePokemonState: (instanceId, updates, isEnemy = false) => {
      if (isEnemy) {
        ...
        set(state => ({
          ...
          activePlayerPokemon:
            state.activePlayerPokemon?.instanceId === instanceId
    ...
    setCommandParseLoadingStatus: status =>
      set({ commandParseLoadingStatus: status }),
    setSelectedItemForAISuggestion: item =>
      set({ selectedItemForAISuggestion: item }),
    setItemActionSuggestions: suggestions =>
      set({ itemActionSuggestions: suggestions }),
