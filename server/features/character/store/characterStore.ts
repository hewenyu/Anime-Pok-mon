...
import { PlayerProfile, Pokemon, InventoryItem, LoadingStatus } from '../../../types';
...
interface CharacterActions {
  ...
  updateInventoryItem: (itemName: string, updates: Partial<InventoryItem>) => void;
...
export const useCharacterStore = create<CharacterState & CharacterActions>((set, get) => ({
  ...
  removePokemonFromTeam: (instanceId) => set((state) => ({
    playerTeam: state.playerTeam.filter(p => p.instanceId !== instanceId)
  })),

  updatePokemonInTeam: (instanceId, updates) => set((state) => ({
  ...
  removeInventoryItem: (itemName, quantity = 1) => set((state) => {
    const existingItemIndex = state.inventory.findIndex(i => i.name === itemName);
    if (existingItemIndex >= 0) {
  ...
  updateInventoryItem: (itemName, updates) => set((state) => ({
  ...
  setCustomizationAssistantResponse: (response) => set({ customizationAssistantResponse: response }),
  setAISuggestedGameStartTime: (time) => set({ aiSuggestedGameStartTime: time }),
  setInitialProfileGenerated: (generated) => set({ initialProfileGenerated: generated }),

  setProfileGenerationLoading: (loading) => set({ profileGenerationLoading: loading }),
